...
3. Obtenha a URL do projeto e a *anon key* pública.
...
2. Vincule o repositório local ao seu projeto do Supabase: `npx supabase link --project-ref <seu_ref_id>`
   *(O `ref_id` é o código de letras aleatórias que aparece na URL do seu painel do Supabase. Por exemplo: se a URL for `https://supabase.com/dashboard/project/abcdefghijk`, o ref_id é `abcdefghijk`)*
...
2. No repositório, vá em **Settings > Pages**. Em *Source*, escolha **GitHub Actions**.
...
4. Lembre-se de adicionar as *Environment Variables* `VITE_SUPABASE_URL` e `VITE_SUPABASE_ANON_KEY` nas configurações de *Secrets and variables* > *Actions* do GitHub, para que elas sejam injetadas no momento do build.
