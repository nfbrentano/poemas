import{a as e,o as t}from"./index-DTI-Pu-u.js";import{n,r,t as i}from"./filter-chips-CCOKHOlj.js";import{t as a}from"./seo-BlITGSmq.js";import{t as o}from"./newsletter-DCsQtgL3.js";var s={meta:{title:`Natanael Brentano - Poemas`},cleanup(){},async render(s,c={}){let l=c.tags?c.tags.split(`,`):[],u=c.cols?c.cols.split(`,`):[],d=c.tag?[decodeURIComponent(c.tag)]:[],f=[...new Set([...l,...d])],p=`Natanael Brentano — Poemas`;if(f.length>0||u.length>0){let e=[];f.length>0&&e.push(`Sentimentos: ${f.join(`, `)}`),u.length>0&&e.push(`Coleções: ${u.join(`, `)}`),p=`${e.join(` | `)} — Natanael Brentano`}a({title:p,description:`Poesia contemporânea e textos curtos sobre o efêmero.`,type:`website`});let m=document.querySelector(`script[id="website-schema"]`);m||(m=document.createElement(`script`),m.id=`website-schema`,m.type=`application/ld+json`,m.textContent=JSON.stringify({"@context":`https://schema.org`,"@type":`WebSite`,url:`https://nfbrentano.github.io/poemas/`,name:`Poemas — Natanael Brentano`,potentialAction:{"@type":`SearchAction`,target:`https://nfbrentano.github.io/poemas/?q={search_term_string}`,"query-input":`required name=search_term_string`}}),document.head.appendChild(m)),s.innerHTML=`
