...
import{i as e,r as t,s as n}from"./index-DUucm8Xb.js";import{t as r}from"./html-BzoIVJF0.js";function i(e,t){let n;return function(...r){clearTimeout(n),n=setTimeout(()=>e.apply(this,r),t)}}var a={meta:{title:`Dashboard Admin`},async render(r,i){let a=new URLSearchParams(window.location.search),o=a.get(`bypass_auth`)===`true`,{data:{session:s}}=await n.auth.getSession();if(!s&&!o){t(`/login`);return}let c=a.get(`view`)||`dashboard`,l=e=>{let t=c===e;return`font-size: 0.85rem; padding: 0.5rem 1rem; color: ${t?`var(--accent-subtle)`:`var(--text-secondary)`}; font-weight: ${t?`500`:`400`}; transition: color var(--transition-fast); border-bottom: 2px solid ${t?`var(--accent-subtle)`:`transparent`}; padding-bottom: 0.25rem; text-decoration: none;`};r.innerHTML=`
    ...
    `,r.querySelector(`#logo-header`).addEventListener(`click`,()=>{t(`/admin`)}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await n.auth.signOut(),t(`/login`)});let u=document.getElementById(`admin-content`);if(c===`dashboard`)await this.renderDashboard(u);else if(c===`list`)await this.renderList(u);else if(c===`collections`)await this.renderCollections(u);else if(c===`editor`)await this.renderEditor(u,a.get(`id`));else if(c===`analytics`){let{default:t}=await e(async()=>{let{default:e}=await import(`./analytics-BN50rxjr.js`);return{default:e}},__vite__mapDeps([0,1,2]));await t.render(u)}else c===`emails`?await this.renderEmailHistory(u):c===`subscribers`?await this.renderSubscribers(u):c===`comments`&&await this.renderComments(u)},async renderDashboard(e){e.innerHTML=`<div class="loading">Carregando painel geral...</div>`;try{let t=[],i=0,a=[],o=[],s=new URLSearchParams(window.location.search).get(`bypass_auth`)===`true`;try{let[e,r,c,l]=await Promise.all([n.from(`poems`).select(`id, title, slug, status, scheduled_at, created_at`),n.from(`poem_comments`).select(`id`).eq(`approved`,!1),n.from(`subscribers`).select(`email, created_at, active`).order(`created_at`,{ascending:!1}).limit(5),n.from(`page_views`).select(`created_at`).gte(`created_at`,new Date(Date.now()-10080*60*1e3).toISOString())]);if(e.error||r.error||c.error||l.error){if(s)throw Error(`Supabase query error, fallback to mock data`);if(e.error)throw e.error;if(r.error)throw r.error;if(c.error)throw c.error;if(l.error)throw l.error}t=e.data||[],i=r.data?.length||0,a=c.data||[],o=l.data||[]}catch(e){if(s){t=[{id:`1`,title:`Poema das Flores`,slug:`poema-das-flores`,status:`published`,created_at:new Date().toISOString()},{id:`2`,title:`Canto Noturno`,slug:`canto-noturno`,status:`draft`,created_at:new Date().toISOString()},{id:`3`,title:`Silêncio da Alma`,slug:`silencio-da-alma`,status:`scheduled`,scheduled_at:new Date(Date.now()+864e5).toISOString(),created_at:new Date().toISOString()}],i=3,a=[{email:`leitor1@exemplo.com`,created_at:new Date(Date.now()-1e5).toISOString(),active:!0},{email:`leitor2@exemplo.com`,created_at:new Date(Date.now()-5e5).toISOString(),active:!1},{email:`leitor3@exemplo.com`,created_at:new Date(Date.now()-9e5).toISOString(),active:!0}],o=[];for(let e=0;e<7;e++){let t=[12,18,5,23,14,30,45][e],n=new Date;n.setDate(n.getDate()-(6-e));for(let e=0;e<t;e++)o.push({created_at:n.toISOString()})}}else throw e}let c=t.length,l=t.filter(e=>e.status===`published`).length,u=t.filter(e=>e.status===`draft`).length,d=t.filter(e=>e.status===`scheduled`).length,f={};o.forEach(e=>{let t=new Date(e.created_at).toISOString().slice(0,10);f[t]=(f[t]||0)+1});let p=[],m=0;for(let e=6;e>=0;e--){let t=new Date;t.setDate(t.getDate()-e);let n=t.toISOString().slice(0,10),r=f[n]||0;m+=r,p.push({label:n,count:r})}let h=t.filter(e=>e.status===`scheduled`&&e.scheduled_at).sort((e,t)=>new Date(e.scheduled_at)-new Date(t.scheduled_at)).slice(0,3);e.innerHTML=`
      ...
      `,e.querySelector(`#new-col-btn`).addEventListener(`click`,()=>i()),e.querySelectorAll(`.edit-col-btn`).forEach(e=>{e.addEventListener(`click`,()=>i(e.dataset.id))}),e.querySelectorAll(`.delete-col-btn`).forEach(e=>{let r=!1;e.addEventListener(`click`,async i=>{if(!r){e.innerText=`Confirmar?`,e.style.color=`#fff`,e.style.backgroundColor=`var(--error)`,e.style.padding=`0.2rem 0.5rem`,e.style.borderRadius=`2px`,r=!0,setTimeout(()=>{e&&(e.innerText=`Excluir`,e.style.color=`var(--error)`,e.style.backgroundColor=`transparent`,e.style.padding=`0`,r=!1)},3e3);return}e.innerText=`Excluindo...`,e.disabled=!0;let{error:a}=await n.from(`collections`).delete().eq(`id`,e.dataset.id);a&&alert(`Erro ao excluir coleção: `+a.message),t()})})},i=async(i=null)=>{e.innerHTML=`<div class="loading">Carregando formulário...</div>`;let a={name:``,slug:``,description:``,image_url:``},o=new Set;try{let[s,c,l]=await Promise.all([n.from(`poems`).select(`id, title, status`).order(`title`,{ascending:!0}),i?n.from(`collections`).select(`*`).eq(`id`,i).single():Promise.resolve({data:null}),i?n.from(`collection_poems`).select(`poem_id`).eq(`collection_id`,i):Promise.resolve({data:[]})]);if(s.error)throw s.error;if(i&&c.error)throw c.error;if(i&&l.error)throw l.error;let u=s.data||[];c.data&&(a=c.data),l.data&&l.data.forEach(e=>o.add(e.poem_id)),e.innerHTML=`
        ...
        `;let d=e.querySelector(`#col-name`),f=e.querySelector(`#col-slug`);d.addEventListener(`input`,()=>{(!i||f.value===``)&&(f.value=d.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``))});let p=e.querySelector(`#checklist-search`),m=e.querySelectorAll(`.checklist-item`);p.addEventListener(`input`,()=>{let e=p.value.toLowerCase().trim();m.forEach(t=>{t.dataset.title.includes(e)?t.style.display=`flex`:t.style.display=`none`})});let h=e.querySelector(`#col-img-url`),g=e.querySelector(`#col-img-preview-container`);h.addEventListener(`input`,()=>{let e=h.value.trim();e?g.innerHTML=`<img src="${e}" id="col-img-preview" style="width: 100%; height: 100%; object-fit: cover;">`:g.innerHTML=`<span id="col-preview-placeholder">Nenhuma imagem</span>`});let _=e.querySelector(`#col-img-upload`),v=e.querySelector(`#upload-status`);_.addEventListener(`change`,async e=>{let t=e.target.files[0];if(t){v.textContent=`Enviando...`,v.style.color=`var(--accent-subtle)`,_.disabled=!0;try{let e=t.name.split(`.`).pop().toLowerCase(),r=`col_cover_${Date.now()}.${e}`,{data:i,error:a}=await n.storage.from(`avatars`).upload(r,t);if(a)throw a;let{data:o}=n.storage.from(`avatars`).getPublicUrl(r),s=o.publicUrl;h.value=s,g.innerHTML=`<img src="${s}" id="col-img-preview" style="width: 100%; height: 100%; object-fit: cover;">`,v.textContent=`Sucesso!`,v.style.color=`var(--success)`}catch(e){console.error(`Erro no upload de capa:`,e),v.textContent=`Erro!`,v.style.color=`var(--error)`}finally{_.disabled=!1,setTimeout(()=>{v.textContent=``},3e3)}}}),e.querySelector(`#cancel-form-btn`).addEventListener(`click`,()=>t()),e.querySelector(`#col-form`).addEventListener(`submit`,async r=>{r.preventDefault();let a=e.querySelector(`#save-col-btn`);a.innerText=`Salvando...`,a.disabled=!0;let o={name:d.value.trim(),slug:f.value.trim(),description:e.querySelector(`#col-description`).value.trim(),image_url:h.value.trim()||null},s=i,c=null;if(i)c=(await n.from(`collections`).update(o).eq(`id`,i)).error;else{let e=await n.from(`collections`).insert([o]).select().single();c=e.error,e.data&&(s=e.data.id)}if(c){alert(`Erro ao salvar coleção: `+c.message),a.innerText=i?`Salvar Alterações`:`Criar Coleção`,a.disabled=!1;return}let l=e.querySelectorAll(`input[name="associated-poems"]:checked`),u=Array.from(l).map(e=>e.value);if(await n.from(`collection_poems`).delete().eq(`collection_id`,s),u.length>0){let e=u.map(e=>({collection_id:s,poem_id:e})),{error:t}=await n.from(`collection_poems`).insert(e);t&&alert(`Coleção salva, mas houve um erro ao associar poemas: `+t.message)}t()})}catch(t){console.error(t),e.innerHTML=`<div class="error">Erro ao carregar o formulário: ${t.message}</div>`}};await t()},async renderList(e){e.innerHTML=`<div class="loading">Carregando obras...</div>`;try{let[a,o]=await Promise.all([n.from(`poems`).select(`id, title, slug, status, published_at, scheduled_at, tags, created_at`).order(`created_at`,{ascending:!1}),n.from(`page_views`).select(`poem_id`)]);if(a.error)throw a.error;let s=a.data||[],c=o.data||[],l={};c.forEach(e=>{e.poem_id&&(l[e.poem_id]=(l[e.poem_id]||0)+1)});let u=new Set;s.forEach(e=>e.tags?.forEach(e=>u.add(e.trim()))),e.innerHTML=`
        <div style="font-family: var(--font-ui); display: grid; gap: var(--space-md);">
