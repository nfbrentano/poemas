...
import{n as e,o as t,r as n}from"./index-DRnHbcne.js";import{t as r}from"./html-BzoIVJF0.js";function i(e,t){let n;return function(...r){clearTimeout(n),n=setTimeout(()=>e.apply(this,r),t)}}var a={meta:{title:`Dashboard Admin`},async render(r,i){let a=new URLSearchParams(window.location.search),o=a.get(`bypass_auth`)===`true`,{data:{session:s}}=await t.auth.getSession();if(!s&&!o){e(`/login`);return}let c=a.get(`view`)||`dashboard`,l=e=>{let t=c===e;return`font-size: 0.85rem; padding: 0.5rem 1rem; color: ${t?`var(--accent-subtle)`:`var(--text-secondary)`}; font-weight: ${t?`500`:`400`}; transition: color var(--transition-fast); border-bottom: 2px solid ${t?`var(--accent-subtle)`:`transparent`}; padding-bottom: 0.25rem; text-decoration: none;`};r.innerHTML=`
    ...
    `,r.querySelector(`#logo-header`).addEventListener(`click`,()=>{e(`/admin`)}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await t.auth.signOut(),e(`/login`)});let u=document.getElementById(`admin-content`);if(c===`dashboard`)await this.renderDashboard(u);else if(c===`list`)await this.renderList(u);else if(c===`collections`)await this.renderCollections(u);else if(c===`editor`)await this.renderEditor(u,a.get(`id`));else if(c===`analytics`){let{default:e}=await n(async()=>{let{default:e}=await import(`./analytics-kNkiAyEA.js`);return{default:e}},__vite__mapDeps([0,1,2]));await e.render(u)}else c===`emails`?await this.renderEmailHistory(u):c===`subscribers`?await this.renderSubscribers(u):c===`comments`&&await this.renderComments(u)},async renderDashboard(e){e.innerHTML=`<div class="loading">Carregando painel geral...</div>`;try{let n=[],i=0,a=[],o=[],s=new URLSearchParams(window.location.search).get(`bypass_auth`)===`true`;try{let[e,r,c,l]=await Promise.all([t.from(`poems`).select(`id, title, slug, status, scheduled_at, created_at`),t.from(`poem_comments`).select(`id`).eq(`approved`,!1),t.from(`subscribers`).select(`email, created_at, active`).order(`created_at`,{ascending:!1}).limit(5),t.from(`page_views`).select(`created_at`).gte(`created_at`,new Date(Date.now()-10080*60*1e3).toISOString())]);if(e.error||r.error||c.error||l.error){if(s)throw Error(`Supabase query error, fallback to mock data`);if(e.error)throw e.error;if(r.error)throw r.error;if(c.error)throw c.error;if(l.error)throw l.error}n=e.data||[],i=r.data?.length||0,a=c.data||[],o=l.data||[]}catch(e){if(s){n=[{id:`1`,title:`Poema das Flores`,slug:`poema-das-flores`,status:`published`,created_at:new Date().toISOString()},{id:`2`,title:`Canto Noturno`,slug:`canto-noturno`,status:`draft`,created_at:new Date().toISOString()},{id:`3`,title:`Silêncio da Alma`,slug:`silencio-da-alma`,status:`scheduled`,scheduled_at:new Date(Date.now()+864e5).toISOString(),created_at:new Date().toISOString()}],i=3,a=[{email:`leitor1@exemplo.com`,created_at:new Date(Date.now()-1e5).toISOString(),active:!0},{email:`leitor2@exemplo.com`,created_at:new Date(Date.now()-5e5).toISOString(),active:!1},{email:`leitor3@exemplo.com`,created_at:new Date(Date.now()-9e5).toISOString(),active:!0}],o=[];for(let e=0;e<7;e++){let t=[12,18,5,23,14,30,45][e],n=new Date;n.setDate(n.getDate()-(6-e));for(let e=0;e<t;e++)o.push({created_at:n.toISOString()})}}else throw e}let c=n.length,l=n.filter(e=>e.status===`published`).length,u=n.filter(e=>e.status===`draft`).length,d=n.filter(e=>e.status===`scheduled`).length,f={};o.forEach(e=>{let t=new Date(e.created_at).toISOString().slice(0,10);f[t]=(f[t]||0)+1});let p=[],m=0;for(let e=6;e>=0;e--){let t=new Date;t.setDate(t.getDate()-e);let n=t.toISOString().slice(0,10),r=f[n]||0;m+=r,p.push({label:n,count:r})}let h=n.filter(e=>e.status===`scheduled`&&e.scheduled_at).sort((e,t)=>new Date(e.scheduled_at)-new Date(t.scheduled_at)).slice(0,3);e.innerHTML=`
      ...
      `,e.querySelector(`#new-col-btn`).addEventListener(`click`,()=>i()),e.querySelectorAll(`.edit-col-btn`).forEach(e=>{e.addEventListener(`click`,()=>i(e.dataset.id))}),e.querySelectorAll(`.delete-col-btn`).forEach(e=>{let r=!1;e.addEventListener(`click`,async i=>{if(!r){e.innerText=`Confirmar?`,e.style.color=`#fff`,e.style.backgroundColor=`var(--error)`,e.style.padding=`0.2rem 0.5rem`,e.style.borderRadius=`2px`,r=!0,setTimeout(()=>{e&&(e.innerText=`Excluir`,e.style.color=`var(--error)`,e.style.backgroundColor=`transparent`,e.style.padding=`0`,r=!1)},3e3);return}e.innerText=`Excluindo...`,e.disabled=!0;let{error:a}=await t.from(`collections`).delete().eq(`id`,e.dataset.id);a&&alert(`Erro ao excluir coleção: `+a.message),n()})})},i=async(i=null)=>{e.innerHTML=`<div class="loading">Carregando formulário...</div>`;let a={name:``,slug:``,description:``,image_url:``},o=new Set;try{let[s,c,l]=await Promise.all([t.from(`poems`).select(`id, title, status`).order(`title`,{ascending:!0}),i?t.from(`collections`).select(`*`).eq(`id`,i).single():Promise.resolve({data:null}),i?t.from(`collection_poems`).select(`poem_id`).eq(`collection_id`,i):Promise.resolve({data:[]})]);if(s.error)throw s.error;if(i&&c.error)throw c.error;if(i&&l.error)throw l.error;let u=s.data||[];c.data&&(a=c.data),l.data&&l.data.forEach(e=>o.add(e.poem_id)),e.innerHTML=`
        ...
        `;let d=e.querySelector(`#col-name`),f=e.querySelector(`#col-slug`);d.addEventListener(`input`,()=>{(!i||f.value===``)&&(f.value=d.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``))});let p=e.querySelector(`#checklist-search`),m=e.querySelectorAll(`.checklist-item`);p.addEventListener(`input`,()=>{let e=p.value.toLowerCase().trim();m.forEach(t=>{t.dataset.title.includes(e)?t.style.display=`flex`:t.style.display=`none`})});let h=e.querySelector(`#col-img-url`),g=e.querySelector(`#col-img-preview-container`);h.addEventListener(`input`,()=>{let e=h.value.trim();e?g.innerHTML=`<img src="${e}" id="col-img-preview" style="width: 100%; height: 100%; object-fit: cover;">`:g.innerHTML=`<span id="col-preview-placeholder">Nenhuma imagem</span>`});let _=e.querySelector(`#col-img-upload`),v=e.querySelector(`#upload-status`);_.addEventListener(`change`,async e=>{let n=e.target.files[0];if(n){v.textContent=`Enviando...`,v.style.color=`var(--accent-subtle)`,_.disabled=!0;try{let e=n.name.split(`.`).pop().toLowerCase(),r=`col_cover_${Date.now()}.${e}`,{data:i,error:a}=await t.storage.from(`avatars`).upload(r,n);if(a)throw a;let{data:o}=t.storage.from(`avatars`).getPublicUrl(r),s=o.publicUrl;h.value=s,g.innerHTML=`<img src="${s}" id="col-img-preview" style="width: 100%; height: 100%; object-fit: cover;">`,v.textContent=`Sucesso!`,v.style.color=`var(--success)`}catch(e){console.error(`Erro no upload de capa:`,e),v.textContent=`Erro!`,v.style.color=`var(--error)`}finally{_.disabled=!1,setTimeout(()=>{v.textContent=``},3e3)}}}),e.querySelector(`#cancel-form-btn`).addEventListener(`click`,()=>n()),e.querySelector(`#col-form`).addEventListener(`submit`,async r=>{r.preventDefault();let a=e.querySelector(`#save-col-btn`);a.innerText=`Salvando...`,a.disabled=!0;let o={name:d.value.trim(),slug:f.value.trim(),description:e.querySelector(`#col-description`).value.trim(),image_url:h.value.trim()||null},s=i,c=null;if(i)c=(await t.from(`collections`).update(o).eq(`id`,i)).error;else{let e=await t.from(`collections`).insert([o]).select().single();c=e.error,e.data&&(s=e.data.id)}if(c){alert(`Erro ao salvar coleção: `+c.message),a.innerText=i?`Salvar Alterações`:`Criar Coleção`,a.disabled=!1;return}let l=e.querySelectorAll(`input[name="associated-poems"]:checked`),u=Array.from(l).map(e=>e.value);if(await t.from(`collection_poems`).delete().eq(`collection_id`,s),u.length>0){let e=u.map(e=>({collection_id:s,poem_id:e})),{error:n}=await t.from(`collection_poems`).insert(e);n&&alert(`Coleção salva, mas houve um erro ao associar poemas: `+n.message)}n()})}catch(t){console.error(t),e.innerHTML=`<div class="error">Erro ao carregar o formulário: ${t.message}</div>`}};await n()},async renderList(n){n.innerHTML=`<div class="loading">Carregando obras...</div>`;try{let[a,o]=await Promise.all([t.from(`poems`).select(`id, title, slug, status, published_at, scheduled_at, tags, created_at`).order(`created_at`,{ascending:!1}),t.from(`page_views`).select(`poem_id`)]);if(a.error)throw a.error;let s=a.data||[],c=o.data||[],l={};c.forEach(e=>{e.poem_id&&(l[e.poem_id]=(l[e.poem_id]||0)+1)});let u=new Set;s.forEach(e=>e.tags?.forEach(e=>u.add(e.trim()))),n.innerHTML=`
        <div style="font-family: var(--font-ui); display: grid; gap: var(--space-md);">
...
`).replace(/<\/?(p|pre|div|span|strong|em|b|i)[^>]*>/gi,``).trim())}n.innerHTML=`
    ...
    `;let o=document.getElementById(`poem-title`),s=document.getElementById(`poem-slug`);o.addEventListener(`input`,()=>{document.getElementById(`preview-title`).innerText=o.value||`Título da Obra`,(!r||s.value===``)&&(s.value=o.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``))});let c=document.getElementById(`poem-content-input`),l=document.getElementById(`preview-content`),u=document.getElementById(`poem-tags`),d=document.getElementById(`poem-status`),f=document.getElementById(`scheduling-fields`);c.addEventListener(`input`,i(()=>{l.innerHTML=c.value},250)),u.addEventListener(`input`,i(()=>{let e=u.value.split(`,`).map(e=>e.trim()).filter(e=>e);document.getElementById(`preview-tags-container`).innerHTML=e.length>0?`<span>•</span> <span>Sentimentos: ${e.join(`, `)}</span>`:``},250)),d.addEventListener(`change`,()=>{f.style.display=d.value===`scheduled`?`block`:`none`});let p=()=>{let e=u.value.split(`,`).map(e=>e.trim()).filter(e=>e),t=document.getElementById(`scheduled-at`);return{title:document.getElementById(`poem-title`).value,slug:document.getElementById(`poem-slug`).value,content:document.getElementById(`poem-content-input`).value,excerpt:document.getElementById(`poem-excerpt`).value,tags:e,status:document.getElementById(`poem-status`).value,scheduled_at:t.value?new Date(t.value).toISOString():null}};document.getElementById(`editor-form`).addEventListener(`submit`,async n=>{n.preventDefault();let i=document.getElementById(`save-btn`);i.innerText=`Salvando...`,i.disabled=!0;let o=p();if(o.status===`scheduled`&&!o.scheduled_at){alert(`Por favor, defina uma data para o agendamento.`),i.innerText=`Gravar Alterações`,i.disabled=!1;return}o.status===`published`&&a.status!==`published`&&(o.published_at=new Date().toISOString());let s=null;if(s=r?(await t.from(`poems`).update(o).eq(`id`,r)).error:(await t.from(`poems`).insert([o])).error,s){console.error(s),alert(`Erro ao salvar: `+s.message),i.innerText=`Gravar Alterações`,i.disabled=!1;return}e(`/admin`)});let m=document.getElementById(`publish-btn`);if(m){let n=!1;m.addEventListener(`click`,async i=>{if(i.preventDefault(),!document.getElementById(`editor-form`).reportValidity())return;if(!n){m.innerText=`Tem certeza? Clique para confirmar.`,m.style.background=`var(--error)`,n=!0,setTimeout(()=>{m&&!m.disabled&&(m.innerText=`Publicar e Notificar Assinantes`,m.style.background=`var(--success)`,n=!1)},4e3);return}m.innerText=`Publicando...`,m.disabled=!0;let a=p();a.status=`published`,a.published_at=new Date().toISOString();let o=r,s=null;if(r)s=(await t.from(`poems`).update(a).eq(`id`,r)).error;else{let e=await t.from(`poems`).insert([a]).select().single();s=e.error,e.data&&(o=e.data.id)}if(s){console.error(s),alert(`Erro ao publicar: `+s.message),m.innerText=`Publicar e Notificar Assinantes`,m.disabled=!1;return}if(o)try{m.innerText=`Enviando newsletter...`;let{data:e,error:n}=await t.functions.invoke(`send-newsletter`,{body:{poemId:o}});if(n)throw n;alert(`Obra publicada e newsletter enviada com sucesso para ${e.count} assinantes!`)}catch(e){console.error(`Newsletter erro:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Obra publicada, mas houve um erro ao enviar a newsletter:\n${t||e.message||`Erro na Edge Function`}`)}e(`/admin`)})}},async renderEmailHistory(e){e.innerHTML=`<div class="loading">Carregando histórico e dados...</div>`;try{let[n,a]=await Promise.all([t.from(`email_campaign_logs`).select(`id, sent_at, status, details, poem_id, poems(title)`).order(`sent_at`,{ascending:!1}),t.from(`poems`).select(`id, title`).eq(`status`,`published`).order(`created_at`,{ascending:!1})]);if(n.error)throw n.error;if(a.error)throw a.error;let o=n.data||[],s=a.data||[],c=o.length,l=o.filter(e=>e.status===`success`).length,u=c>0?(l/c*100).toFixed(1):`100.0`,d=o[0]||null;e.innerHTML=`
      ...
      `;let f=e.querySelector(`#email-tbody`),p=e.querySelector(`#results-count`),m=e.querySelector(`#manual-dispatch-modal`),h=e.querySelector(`#log-details-modal`);e.querySelector(`#open-dispatch-modal-btn`).addEventListener(`click`,()=>{m.style.display=`flex`,e.querySelector(`#manual-dispatch-form`).reset()}),e.querySelector(`#close-dispatch-modal-btn`).addEventListener(`click`,()=>{m.style.display=`none`}),e.querySelector(`#close-details-modal-btn`).addEventListener(`click`,()=>{h.style.display=`none`}),[m,h].forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&(e.style.display=`none`)})});let g=null,_=async(n,r,i)=>{if(!n){alert(`Não é possível reenviar: id da obra indisponível.`);return}if(!confirm(`Deseja reenviar a newsletter da obra "${r}" para todos os assinantes ativos?`))return;let a=i.innerText;i.disabled=!0,i.innerText=`Enviando...`,i.style.opacity=`0.5`;try{let{data:i,error:a}=await t.functions.invoke(`send-newsletter`,{body:{poemId:n}});if(a)throw a;alert(`Newsletter para "${r}" reenviada com sucesso para ${i.count} assinantes!`),h.style.display=`none`,this.renderEmailHistory(e)}catch(e){console.error(`Erro ao reenviar:`,e),alert(`Erro ao enviar newsletter:\n${e.message||`Erro na Edge Function`}`),i.disabled=!1,i.innerText=a,i.style.opacity=`1`}};e.querySelector(`#resend-from-modal-btn`).addEventListener(`click`,async e=>{e.preventDefault(),g&&g.poem_id&&await _(g.poem_id,g.poems?.title||`Desconhecido`,e.currentTarget)});let v=t=>{if(p.innerText=t.length,t.length===0){f.innerHTML=`
            <tr>
              <td colspan="5" style="padding: var(--space-xl) 0; text-align: center; color: var(--text-muted); font-style: italic;">
...
`),t=encodeURI(e),r=document.createElement(`a`);r.setAttribute(`href`,t),r.setAttribute(`download`,`assinantes_${new Date().toISOString().split(`T`)[0]}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)})},async renderComments(e){e.innerHTML=`<div class="loading">Carregando comentários...</div>`;let{data:n,error:i}=await t.from(`poem_comments`).select(`id, author_name, content, approved, created_at, poems(title)`).order(`created_at`,{ascending:!1});if(i){e.innerHTML=`<div class="error">Erro ao carregar: ${i.message}</div>`;return}if(!n||n.length===0){e.innerHTML=`<p>Nenhum comentário encontrado.</p>`;return}e.innerHTML=`
      <table style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead>
          <tr style="border-bottom: 1px solid var(--border-strong); color: var(--text-secondary); font-family: var(--font-ui); font-size: 0.8rem; text-transform: uppercase; letter-spacing: 1px;">
    ...
    `,e.querySelectorAll(`.approve-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{let r=n.dataset.id,{error:i}=await t.from(`poem_comments`).update({approved:!0}).eq(`id`,r);i?alert(`Erro ao aprovar: `+i.message):this.renderComments(e)})}),e.querySelectorAll(`.delete-comment-btn`).forEach(n=>{n.addEventListener(`click`,async()=>{if(!confirm(`Excluir este comentário?`))return;let r=n.dataset.id,{error:i}=await t.from(`poem_comments`).delete().eq(`id`,r);i?alert(`Erro ao excluir: `+i.message):this.renderComments(e)})})}};export{a as default};