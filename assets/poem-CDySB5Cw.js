...
import{i as e,n as t,o as n,r,t as i}from"./index-DRnHbcne.js";import{t as a}from"./html-BzoIVJF0.js";import{t as o}from"./seo-Cx_atl1Z.js";import{t as s}from"./newsletter-xwzdDNSF.js";var c=[`🕯️`,`💧`,`🌿`,`🌙`,`✨`];function l(){let e=localStorage.getItem(`reaction_session_id`);return e||(e=typeof crypto<`u`&&crypto.randomUUID?crypto.randomUUID():`xxx-xx-4xxx-yxx-xxxxxxxxxxxx`.replace(/[xy]/g,e=>{let t=Math.random()*16|0;return(e===`x`?t:t&3|8).toString(16)}),localStorage.setItem(`reaction_session_id`,e)),e}async function u(e){let{data:t,error:r}=await n.from(`poem_reactions`).select(`emoji, session_id`).eq(`poem_id`,e);if(r)return{counts:{},userReactions:new Set};let i=l(),a={},o=new Set;return c.forEach(e=>a[e]=0),(t||[]).forEach(e=>{a[e.emoji]=(a[e.emoji]||0)+1,e.session_id===i&&o.add(e.emoji)}),{counts:a,userReactions:o}}async function d(e,t){let r=l(),{data:i}=await n.from(`poem_reactions`).select(`id`).eq(`poem_id`,e).eq(`session_id`,r).eq(`emoji`,t).maybeSingle();return i?(await n.from(`poem_reactions`).delete().eq(`id`,i.id),`removed`):(await n.from(`poem_reactions`).insert({poem_id:e,emoji:t,session_id:r}),`added`)}function ee(e,t){let n;return function(){let r=arguments,i=this;n||(e.apply(i,r),n=!0,setTimeout(()=>n=!1,t))}}var f=null,p=null,m=null,h=null,g={meta:{title:`Poema`},cleanup(){f&&window.removeEventListener(`scroll`,f),p&&document.removeEventListener(`touchstart`,p),m&&document.removeEventListener(`touchend`,m),h&&document.removeEventListener(`keydown`,h),document.documentElement.classList.remove(`immersive-mode`),f=null,p=null,m=null,h=null},async render(l,g){let _=g.slug;l.innerHTML=`
    ...
    `,console.log(`[Poem] Fetching slug:`,_);let{data:v,error:y}=await n.rpc(`get_poem_with_navigation`,{target_slug:_});y&&console.error(`[Poem] RPC Error:`,y);let b=v&&Array.isArray(v)&&v.length>0?v[0]:null;if(y||!b){console.warn(`[Poem] Poem not found or error occurred`),l.innerHTML=`
      ...
      `;return}let x=b.prev_slug,S=b.next_slug,C=b.prev_title,w=b.next_title,te=(b.content||``).replace(/<[^>]*>/g,` `).replace(/\s+/g,` `).trim().split(` `).filter(e=>e.length>0).length,T=Math.ceil(te/200),ne=T<=1?`1 min de leitura`:`${T} min de leitura`;e(`/poema/`+b.slug,b.id);let re=window.location.href,ie=(b.excerpt||b.content).replace(/<[^>]*>?/gm,``).replace(/\s+/g,` `).trim().slice(0,160)+`...`;o({title:b.title,description:ie,url:re,type:`article`,publishedTime:b.published_at,tags:b.tags});let{data:{session:ae}}=await n.auth.getSession(),E=!!ae,oe=(e=>e?e.split(/\n\s*\n/).filter(e=>e.trim()).map(e=>`<div class="stanza stagger-reveal">${e}</div>`).join(``):``)(b.content);l.innerHTML=`
        ...
        <article class="single-poem fade-in">
          <header>
            ...
            <div class="poem-meta">
              ...
              <span>•</span>
              <span class="reading-time">${ne}</span>
            </div>
          </header>

          
          
          <div id="poem-text" class="poem-content">${oe}</div>
