import{o as e}from"./index-DRnHbcne.js";function t(e,t=700,n=160){if(!e||e.length===0)return`<p style="color:var(--text-muted);font-family:var(--font-ui);font-size:0.85rem;">Sem dados para o período.</p>`;let r=e.map(e=>e.count),i=Math.max(...r,1),a=r.reduce((e,t)=>e+t,0),o=Math.round(a/(r.length||1)),s=t-36-16,c=n-16-32,l=e.length,u=e=>36+e/(l-1||1)*s,d=e=>16+c-e/i*c,f=d(o),p=e.map((e,t)=>`${u(t)},${d(e.count)}`).join(` `),m=`36,${16+c} `+p+` ${u(l-1)},${16+c}`,h=Array.from({length:5},(e,t)=>{let n=Math.round(i/4*t),r=d(n);return`<line x1="36" y1="${r}" x2="${36+s}" y2="${r}" stroke="var(--border-subtle)" stroke-width="1"/>
