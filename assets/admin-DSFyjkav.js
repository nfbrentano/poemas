const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/analytics-2LDRSkdu.js","assets/supabase-Cmo813Sf.js"])))=>i.map(i=>d[i]);
import{r as e,t}from"./index-mEssQEaL.js";import{supabase as n}from"./supabase-Cmo813Sf.js";import{n as r,r as i,t as a}from"./html-Ca31mHym.js";function o(e,t){let n;return function(...r){clearTimeout(n),n=setTimeout(()=>e.apply(this,r),t)}}var s={meta:{title:`Dashboard Admin`},async render(r,i){let a=new URLSearchParams(window.location.search),o=a.get(`bypass_auth`)===`true`,{data:{session:s}}=await n.auth.getSession();if(!s&&!o){t(`/login`);return}let c=a.get(`view`)||`dashboard`,l=e=>{let t=c===e;return`font-size: 0.85rem; padding: 0.5rem 1rem; color: ${t?`var(--accent-subtle)`:`var(--text-secondary)`}; font-weight: ${t?`500`:`400`}; transition: color var(--transition-fast); border-bottom: 2px solid ${t?`var(--accent-subtle)`:`transparent`}; padding-bottom: 0.25rem; text-decoration: none;`};r.innerHTML=`
    ...
    `,r.querySelector(`#logo-header`).addEventListener(`click`,()=>{t(`/admin`)}),document.getElementById(`logout-btn`).addEventListener(`click`,async()=>{await n.auth.signOut(),t(`/login`)});let u=document.getElementById(`admin-content`);if(c===`dashboard`)await this.renderDashboard(u);else if(c===`list`)await this.renderList(u);else if(c===`collections`)await this.renderCollections(u);else if(c===`editor`)await this.renderEditor(u,a.get(`id`));else if(c===`analytics`){let{default:t}=await e(async()=>{let{default:e}=await import(`./analytics-2LDRSkdu.js`);return{default:e}},__vite__mapDeps([0,1]));await t.render(u)}else c===`emails`?await this.renderEmailHistory(u):c===`subscribers`?await this.renderSubscribers(u):c===`comments`&&await this.renderComments(u)},async renderDashboard(e){e.innerHTML=`<div class="loading">Carregando painel geral...</div>`;try{let t=[],r=0,i=[],o=[],s=new URLSearchParams(window.location.search).get(`bypass_auth`)===`true`;try{let[e,a,c,l]=await Promise.all([n.from(`poems`).select(`id, title, slug, status, scheduled_at, created_at`),n.from(`poem_comments`).select(`id`).eq(`approved`,!1),n.from(`subscribers`).select(`email, created_at, active`).order(`created_at`,{ascending:!1}).limit(5),n.from(`page_views`).select(`created_at`).gte(`created_at`,new Date(Date.now()-6048e5).toISOString())]);if(e.error||a.error||c.error||l.error){if(s)throw Error(`Supabase query error, fallback to mock data`);if(e.error)throw e.error;if(a.error)throw a.error;if(c.error)throw c.error;if(l.error)throw l.error}t=e.data||[],r=a.data?.length||0,i=c.data||[],o=l.data||[]}catch(e){if(s){t=[{id:`1`,title:`Poema das Flores`,slug:`poema-das-flores`,status:`published`,created_at:new Date().toISOString()},{id:`2`,title:`Canto Noturno`,slug:`canto-noturno`,status:`draft`,created_at:new Date().toISOString()},{id:`3`,title:`Silêncio da Alma`,slug:`silencio-da-alma`,status:`scheduled`,scheduled_at:new Date(Date.now()+864e5).toISOString(),created_at:new Date().toISOString()}],r=3,i=[{email:`leitor1@exemplo.com`,created_at:new Date(Date.now()-1e5).toISOString(),active:!0},{email:`leitor2@exemplo.com`,created_at:new Date(Date.now()-5e5).toISOString(),active:!1},{email:`leitor3@exemplo.com`,created_at:new Date(Date.now()-9e5).toISOString(),active:!0}],o=[];for(let e=0;e<7;e++){let t=[12,18,5,23,14,30,45][e],n=new Date;n.setDate(n.getDate()-(6-e));for(let e=0;e<t;e++)o.push({created_at:n.toISOString()})}}else throw e}let c=t.length,l=t.filter(e=>e.status===`published`).length,u=t.filter(e=>e.status===`draft`).length,d=t.filter(e=>e.status===`scheduled`).length,f={};o.forEach(e=>{let t=new Date(e.created_at).toISOString().slice(0,10);f[t]=(f[t]||0)+1});let p=[],m=0;for(let e=6;e>=0;e--){let t=new Date;t.setDate(t.getDate()-e);let n=t.toISOString().slice(0,10),r=f[n]||0;m+=r,p.push({label:n,count:r})}let h=t.filter(e=>e.status===`scheduled`&&e.scheduled_at).sort((e,t)=>new Date(e.scheduled_at)-new Date(t.scheduled_at)).slice(0,3);e.innerHTML=`
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: var(--space-md); width: 100%;">
            ...
            <a href="/poemas/admin?view=comments" data-link style="background: var(--bg-elevated); padding: var(--space-md); border-radius: 4px; border: 1px solid var(--border-subtle); display: flex; flex-direction: column; justify-content: space-between; min-height: 110px; height: auto; box-sizing: border-box; text-decoration: none; transition: border-color var(--transition-fast);" onmouseover="this.style.borderColor='var(--border-strong)'" onmouseout="this.style.borderColor='var(--border-subtle)'">
              ...
              <div style="display: flex; align-items: center; justify-content: space-between; margin-top: auto; width: 100%;">
                <span style="font-size: 2.2rem; font-family: var(--font-display); color: ${r>0?`var(--error)`:`var(--text-muted)`}; font-weight: 400; line-height: 1;">${r}</span>
                ${r>0?`<span style="background: rgba(204, 74, 74, 0.15); color: var(--error); padding: 0.2rem 0.5rem; border-radius: 2px; font-size: 0.75rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">Revisar</span>`:`<span style="color: var(--success); font-size: 0.75rem; font-weight: 500;">✔ Tudo limpo</span>`}
              </div>
            </a>
                    ...
                    <tbody>
                      ${i.map(e=>`
                        <tr style="border-bottom: 1px solid var(--border-subtle);">
                          <td style="padding: var(--space-xs) var(--space-xs) var(--space-xs) 0; color: var(--text-primary); font-family: var(--font-ui); font-size: 0.8rem; word-break: break-all;">${a(e.email)}</td>
                  ...
                  <div style="display: grid; gap: var(--space-xs);">
                    ${h.map(e=>`
                      <div style="padding: var(--space-xs); border: 1px solid var(--border-subtle); border-radius: 2px; background: rgba(255, 255, 255, 0.01); display: flex; justify-content: space-between; align-items: center;">
                        <div>
                          <div style="font-family: var(--font-display); font-size: 1rem; color: var(--text-primary); font-weight: 400;">${a(e.title)}</div>
                          <div style="font-size: 0.75rem; color: var(--text-muted); font-family: var(--font-ui); margin-top: 2px;">Slug: ${a(e.slug)}</div>
                        </div>
      ...
      `;let g=e.querySelector(`#visits-kpi-card`);if(g){let e=g.querySelector(`#sparkline-svg`),t=g.querySelector(`#sparkline-tracker`),n=g.querySelector(`#sparkline-hover-dot`),r=g.querySelector(`#sparkline-tooltip`),i=i=>{let o=e.getBoundingClientRect(),s=i.clientX-o.left;if(s<-10||s>o.width+10){a();return}let c=Math.min(6,Math.max(0,Math.round(s/o.width*6))),l=p[c];if(!l)return;let u=Math.max(...p.map(e=>e.count),1),d=c/6*160,f=38-l.count/u*36;t.setAttribute(`x1`,d),t.setAttribute(`x2`,d),t.style.display=`block`,n.setAttribute(`cx`,d),n.setAttribute(`cy`,f),n.style.display=`block`;let m=new Date(l.label+`T00:00:00`),h=m.toLocaleDateString(`pt-BR`,{day:`2-digit`,month:`2-digit`}),_=m.toLocaleDateString(`pt-BR`,{weekday:`short`}).replace(`.`,``);r.style.display=`block`,r.innerHTML=`
        ...
        <div style="background: var(--bg-elevated); border: 1px solid var(--border-subtle); border-radius: 4px; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
          ${t?`
            <div style="height: 120px; background-image: url('${a(t)}'); background-size: cover; background-position: center; border-bottom: 1px solid var(--border-subtle);"></div>
          `:`
          ...
          <div style="padding: var(--space-md); flex-grow: 1; display: flex; flex-direction: column; justify-content: space-between; gap: var(--space-xs);">
            <div>
              <div style="display: flex; justify-content: space-between; align-items: baseline; gap: 8px;">
                <h4 style="font-family: var(--font-display); font-size: 1.25rem; font-weight: 400; margin: 0; color: var(--text-primary);">${a(e.name)}</h4>
              ...
              <div style="font-size: 0.75rem; color: var(--text-muted); font-family: var(--font-ui); margin-top: 4px;">Slug: ${a(e.slug)}</div>
              <p style="color: var(--text-secondary); font-size: 0.85rem; margin-top: var(--space-2xs); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4;">${a(e.description||`Sem descrição.`)}</p>
            </div>
      ...
      `,e.querySelector(`#new-col-btn`).addEventListener(`click`,()=>i()),e.querySelectorAll(`.edit-col-btn`).forEach(e=>{e.addEventListener(`click`,()=>i(e.dataset.id))}),e.querySelectorAll(`.delete-col-btn`).forEach(e=>{let r=!1;e.addEventListener(`click`,async i=>{if(!r){e.innerText=`Confirmar?`,e.style.color=`#fff`,e.style.backgroundColor=`var(--error)`,e.style.padding=`0.2rem 0.5rem`,e.style.borderRadius=`2px`,r=!0,setTimeout(()=>{e&&(e.innerText=`Excluir`,e.style.color=`var(--error)`,e.style.backgroundColor=`transparent`,e.style.padding=`0`,r=!1)},3e3);return}e.innerText=`Excluindo...`,e.disabled=!0;let{error:a}=await n.from(`collections`).delete().eq(`id`,e.dataset.id);a&&alert(`Erro ao excluir coleção: `+a.message),t()})})},i=async(i=null)=>{e.innerHTML=`<div class="loading">Carregando formulário...</div>`;let o={name:``,slug:``,description:``,image_url:``},s=new Set;try{let[c,l,u]=await Promise.all([n.from(`poems`).select(`id, title, status`).order(`title`,{ascending:!0}),i?n.from(`collections`).select(`*`).eq(`id`,i).single():Promise.resolve({data:null}),i?n.from(`collection_poems`).select(`poem_id`).eq(`collection_id`,i):Promise.resolve({data:[]})]);if(c.error)throw c.error;if(i&&l.error)throw l.error;if(i&&u.error)throw u.error;let d=c.data||[];l.data&&(o=l.data),u.data&&u.data.forEach(e=>s.add(e.poem_id)),e.innerHTML=`
            ...
            <form id="col-form" style="display: grid; gap: var(--space-md);">
              <div style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--space-md);">
                <div>
                  ...
                  <input type="text" id="col-name" value="${a(o.name)}" required style="width: 100%; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px;">
                </div>
                <div>
                  ...
                  <input type="text" id="col-slug" value="${a(o.slug)}" required style="width: 100%; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px;">
                </div>
              </div>
              ...
              <div>
                ...
                <textarea id="col-description" style="width: 100%; min-height: 80px; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px; resize: vertical;">${a(o.description||``)}</textarea>
              </div>
              ...
              <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: var(--space-md); align-items: start; flex-wrap: wrap;">
                <div>
                  ...
                  <input type="text" id="col-img-url" value="${a(o.image_url||``)}" placeholder="https://exemplo.com/imagem.jpg" style="width: 100%; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px; margin-bottom: 8px;">
                ...
                <div>
                  ...
                  <div id="col-img-preview-container" style="width: 100%; height: 105px; border: 1px solid var(--border-strong); border-radius: 2px; background: var(--bg-secondary); display: flex; align-items: center; justify-content: center; overflow: hidden; color: var(--text-muted); font-size: 0.8rem;">
                    ${r(o.image_url)?`
                      <img src="${a(r(o.image_url))}" id="col-img-preview" style="width: 100%; height: 100%; object-fit: cover;">
                    `:`
              ...
              <div>
                ...
                <div id="checklist-container" style="max-height: 200px; overflow-y: auto; border: 1px solid var(--border-strong); border-radius: 2px; padding: var(--space-2xs); background: var(--bg-secondary); display: grid; gap: 4px;">
                  ${d.map(e=>{let t=s.has(e.id);return`
                      <label class="checklist-item" data-title="${e.title.toLowerCase()}" style="display: flex; align-items: center; gap: var(--space-2xs); padding: var(--space-3xs) var(--space-2xs); cursor: pointer; border-radius: 2px; transition: background-color var(--transition-fast);">
                        ...
                        <span style="font-size: 0.9rem; color: var(--text-primary);">${a(e.title)}</span>
        ...
        `;let f=e.querySelector(`#col-name`),p=e.querySelector(`#col-slug`);f.addEventListener(`input`,()=>{(!i||p.value===``)&&(p.value=f.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``))});let m=e.querySelector(`#checklist-search`),h=e.querySelectorAll(`.checklist-item`);m.addEventListener(`input`,()=>{let e=m.value.toLowerCase().trim();h.forEach(t=>{t.dataset.title.includes(e)?t.style.display=`flex`:t.style.display=`none`})});let g=e.querySelector(`#col-img-url`),_=e.querySelector(`#col-img-preview-container`),v=e=>{_.replaceChildren();let t=r(e);if(t){let e=document.createElement(`img`);e.id=`col-img-preview`,e.setAttribute(`src`,encodeURI(t)),e.style.width=`100%`,e.style.height=`100%`,e.style.objectFit=`cover`,_.appendChild(e)}else{let e=document.createElement(`span`);e.id=`col-preview-placeholder`,e.textContent=`Nenhuma imagem`,_.appendChild(e)}};g.addEventListener(`input`,()=>{v(g.value.trim())});let y=e.querySelector(`#col-img-upload`),b=e.querySelector(`#upload-status`);y.addEventListener(`change`,async e=>{let t=e.target.files[0];if(t){b.textContent=`Enviando...`,b.style.color=`var(--accent-subtle)`,y.disabled=!0;try{let e=t.name.split(`.`).pop().toLowerCase(),r=`col_cover_${Date.now()}.${e}`,{data:i,error:a}=await n.storage.from(`avatars`).upload(r,t);if(a)throw a;let{data:o}=n.storage.from(`avatars`).getPublicUrl(r),s=o.publicUrl;g.value=s,v(s),b.textContent=`Sucesso!`,b.style.color=`var(--success)`}catch(e){console.error(`Erro no upload de capa:`,e),b.textContent=`Erro!`,b.style.color=`var(--error)`}finally{y.disabled=!1,setTimeout(()=>{b.textContent=``},3e3)}}}),e.querySelector(`#cancel-form-btn`).addEventListener(`click`,()=>t()),e.querySelector(`#col-form`).addEventListener(`submit`,async r=>{r.preventDefault();let a=e.querySelector(`#save-col-btn`);a.innerText=`Salvando...`,a.disabled=!0;let o={name:f.value.trim(),slug:p.value.trim(),description:e.querySelector(`#col-description`).value.trim(),image_url:g.value.trim()||null},s=i,c=null;if(i)c=(await n.from(`collections`).update(o).eq(`id`,i)).error;else{let e=await n.from(`collections`).insert([o]).select().single();c=e.error,e.data&&(s=e.data.id)}if(c){alert(`Erro ao salvar coleção: `+c.message),a.innerText=i?`Salvar Alterações`:`Criar Coleção`,a.disabled=!1;return}let l=e.querySelectorAll(`input[name="associated-poems"]:checked`),u=Array.from(l).map(e=>e.value);if(await n.from(`collection_poems`).delete().eq(`collection_id`,s),u.length>0){let e=u.map(e=>({collection_id:s,poem_id:e})),{error:t}=await n.from(`collection_poems`).insert(e);t&&alert(`Coleção salva, mas houve um erro ao associar poemas: `+t.message)}t()})}catch(t){console.error(t),e.innerHTML=`<div class="error">Erro ao carregar o formulário: ${t.message}</div>`}};await t()},async renderList(e){e.innerHTML=`<div class="loading">Carregando obras...</div>`;try{let[r,i]=await Promise.all([n.from(`poems`).select(`id, title, slug, status, published_at, scheduled_at, tags, created_at`).order(`created_at`,{ascending:!1}),n.from(`page_views`).select(`poem_id`)]);if(r.error)throw r.error;let s=r.data||[],c=i.data||[],l={};c.forEach(e=>{e.poem_id&&(l[e.poem_id]=(l[e.poem_id]||0)+1)});let u=new Set;s.forEach(e=>e.tags?.forEach(e=>u.add(e.trim()))),e.innerHTML=`
        <div style="font-family: var(--font-ui); display: grid; gap: var(--space-md);">
          ...
          <div style="display: flex; gap: var(--space-xs); align-items: center; flex-wrap: wrap; background: var(--bg-elevated); padding: var(--space-sm); border-radius: 4px; border: 1px solid var(--border-subtle);">
            ...
            <select id="list-filter-tag" style="padding: 0.5rem var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px; font-size: 0.85rem; max-width: 180px; cursor: pointer;">
              ...
              ${Array.from(u).sort().map(e=>`<option value="${a(e)}">${a(e)}</option>`).join(``)}
            </select>
      ...
      `;let d=e.querySelector(`#list-tbody`),f=e.querySelector(`#results-count`),p=e=>{if(f.innerText=e.length,e.length===0){d.innerHTML=`
            <tr>
          ...
          `;return}d.innerHTML=e.map(e=>{let t=l[e.id]||0,n=``;n=e.status===`scheduled`?`Agendado • ${new Date(e.scheduled_at).toLocaleString(`pt-BR`,{dateStyle:`short`,timeStyle:`short`})}`:e.status===`published`?`Publicado • ${new Date(e.published_at).toLocaleDateString(`pt-BR`)}`:`Criado • ${new Date(e.created_at).toLocaleDateString(`pt-BR`)}`;let r=e.status===`published`?`var(--success)`:e.status===`scheduled`?`var(--accent-subtle)`:`var(--border-strong)`,i=e.status===`published`?`var(--success)`:e.status===`scheduled`?`var(--accent-subtle)`:`var(--text-muted)`;return`
            <tr style="border-bottom: 1px solid var(--border-subtle); transition: background-color var(--transition-fast);">
              <td style="padding: var(--space-md) 0; font-family: var(--font-display); font-size: 1.2rem; color: var(--text-primary);">${a(e.title)}</td>
              <td style="padding: var(--space-md) 0; font-family: var(--font-ui); color: var(--text-muted); font-size: 0.85rem;">${a(e.slug)}</td>
          ...
          `}).join(``),d.querySelectorAll(`.delete-btn`).forEach(e=>{let r=!1;e.addEventListener(`click`,async i=>{i.preventDefault();let a=e.dataset.id;if(!r){let t=e.innerText;e.innerText=`Tem certeza?`,e.style.color=`#fff`,e.style.backgroundColor=`var(--error)`,e.style.padding=`0.2rem 0.5rem`,e.style.borderRadius=`2px`,e.style.opacity=`1`,r=!0,setTimeout(()=>{e&&!e.disabled&&(e.innerText=t,e.style.color=`var(--error)`,e.style.backgroundColor=`transparent`,e.style.padding=`0`,e.style.opacity=`0.7`,r=!1)},3e3);return}e.innerText=`Excluindo...`,e.disabled=!0;let{error:o}=await n.from(`poems`).delete().eq(`id`,a);if(o){console.error(o),alert(`Erro ao excluir: `+o.message),e.innerText=`Excluir`,e.disabled=!1;return}t(`/admin?view=list`)})})},m=e.querySelector(`#list-search`),h=e.querySelector(`#list-filter-status`),g=e.querySelector(`#list-filter-tag`),_=e.querySelector(`#list-sort`),v=()=>{let e=m.value.toLowerCase().trim(),t=h.value,n=g.value,r=_.value,i=[...s];e&&(i=i.filter(t=>t.title.toLowerCase().includes(e)||t.slug.toLowerCase().includes(e))),t!==`all`&&(i=i.filter(e=>e.status===t)),n!==`all`&&(i=i.filter(e=>e.tags&&e.tags.includes(n))),r===`newest`?i.sort((e,t)=>new Date(t.published_at||t.created_at)-new Date(e.published_at||e.created_at)):r===`oldest`?i.sort((e,t)=>new Date(e.published_at||e.created_at)-new Date(t.published_at||t.created_at)):r===`title-az`?i.sort((e,t)=>e.title.localeCompare(t.title)):r===`title-za`?i.sort((e,t)=>t.title.localeCompare(e.title)):r===`views`&&i.sort((e,t)=>(l[t.id]||0)-(l[e.id]||0)),p(i)};m.addEventListener(`input`,o(v,150)),h.addEventListener(`change`,v),g.addEventListener(`change`,v),_.addEventListener(`change`,v),v()}catch(t){console.error(t),e.innerHTML=`<div class="error">Erro ao carregar obras: ${t.message}</div>`}},async renderEditor(e,s){let c={title:``,slug:``,content:``,excerpt:``,tags:[],status:`draft`,audio_url:``};if(s){e.innerHTML=`<div class="loading">Carregando poema...</div>`;let{data:t}=await n.from(`poems`).select(`*`).eq(`id`,s).single();t&&(c=t,c.content=i(c.content))}e.innerHTML=`
        ...
        <div class="editor-layout">
          <div class="editor-pane">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--space-lg);">
              <div>
                ...
                <input type="text" id="poem-title" value="${a(c.title)}" required style="width: 100%; font-size: 1.5rem; font-family: var(--font-display); padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0;">
              </div>
              <div>
                ...
                <input type="text" id="poem-slug" value="${a(c.slug)}" required style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0; color: var(--text-muted);">
              </div>
            </div>
            ...
            <div style="margin-top: var(--space-md);">
              ...
              <textarea id="poem-content-input" required style="width: 100%; min-height: 500px; font-family: var(--font-body); font-size: 1.1rem; line-height: 1.6; padding: var(--space-md); border: 1px solid var(--border-strong); background: var(--bg-primary); border-radius: 2px; color: var(--text-primary); resize: vertical;">${a(c.content)}</textarea>
            </div>
            ...
            <div>
              ...
              <textarea id="poem-excerpt" style="width: 100%; min-height: 80px; font-family: var(--font-body); font-size: 1rem; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); border-radius: 2px; resize: vertical;">${a(c.excerpt||``)}</textarea>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-lg);">
              <div>
                ...
                <input type="text" id="poem-tags" value="${a(c.tags?c.tags.join(`, `):``)}" placeholder="Ex: Amor, Saudade, Melancolia" style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0;">
              </div>
              <div>
                ...
                <select id="poem-status" style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0; color: var(--text-primary);">
                  ...
                  <option value="scheduled" ${c.status===`scheduled`?`selected`:``}>Agendado</option>
            ...
            <div class="admin-audio-card">
              ...
              <input type="hidden" id="poem-audio-url" value="${a(c.audio_url||``)}">
              ...
              <div style="display: flex; gap: var(--space-sm); align-items: center; flex-wrap: wrap; margin-top: var(--space-2xs);">
                ...
                <button type="button" id="remove-audio-btn" class="btn-secondary" style="padding: 0.5rem 1rem; font-size: 0.85rem; color: var(--error); border-color: var(--error); display: ${c.audio_url?`inline-block`:`none`};">
                  Remover Áudio
                </button>
              </div>
              ...
              <div id="audio-preview-container" class="admin-audio-preview" style="display: ${c.audio_url?`flex`:`none`};">
                <div style="display: flex; align-items: center; gap: var(--space-xs); flex: 1; min-width: 200px;">
                  ...
                  <audio id="audio-preview-player" controls src="${r(c.audio_url||``)}" style="height: 32px; width: 100%; max-width: 400px;"></audio>
                </div>
                <span id="audio-file-name" style="font-size: 0.75rem; color: var(--text-muted); word-break: break-all;">
                  ${c.audio_url?`Áudio vinculado`:``}
                </span>
              </div>
            </div>

            <div id="scheduling-fields" style="margin-top: var(--space-md); ${c.status===`scheduled`?``:`display: none;`}">
              ...
              <input type="datetime-local" id="scheduled-at" value="${c.scheduled_at?new Date(c.scheduled_at).toISOString().slice(0,16):``}" style="width: 100%; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px;">
              <p class="field-help">Se definido e o status for "Agendado", o poema será publicado automaticamente.</p>
              ${c.status===`scheduled`?`<p class="field-help" style="color: var(--accent-subtle); font-style: italic;">Este poema será publicado automaticamente em ${new Date(c.scheduled_at).toLocaleString(`pt-BR`)}.</p>`:``}
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: var(--space-md); margin-top: var(--space-lg); border-top: 1px solid var(--border-subtle); padding-top: var(--space-lg);">
              ...
              ${c.status===`draft`?`<button type="button" class="btn-primary" id="publish-btn" style="padding: 0.75rem 1.5rem; background: var(--success); color: #fff; border-radius: 2px; font-weight: 500;">Publicar Agora</button>`:``}
            </div>
          </div>
          ...
          <div class="preview-pane">
            ...
            <article class="preview-poem">
              <h1 id="preview-title">${a(c.title||`Título da Obra`)}</h1>
              <div class="poem-meta preview-meta">
                <span id="preview-date">${c.published_at?new Date(c.published_at).toLocaleDateString(`pt-BR`):new Date().toLocaleDateString(`pt-BR`)}</span>
                <span id="preview-tags-container">${c.tags&&c.tags.length>0?`<span>•</span> <span>Sentimentos: ${a(c.tags.join(`, `))}</span>`:``}</span>
              </div>
              <div id="preview-content" class="poem-content">${a(c.content||``)}</div>
            </article>
          </div>
        </div>
      </form>
    `;let l=document.getElementById(`poem-title`),u=document.getElementById(`poem-slug`);l.addEventListener(`input`,()=>{if(document.getElementById(`preview-title`).textContent=l.value||`Título da Obra`,!s||u.value===``){let e=l.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``);u.value=e}});let d=document.getElementById(`poem-content-input`),f=document.getElementById(`preview-content`),p=document.getElementById(`poem-tags`),m=document.getElementById(`poem-status`),h=document.getElementById(`scheduling-fields`);d.addEventListener(`input`,o(()=>{f.textContent=d.value},250)),p.addEventListener(`input`,o(()=>{let e=p.value.split(`,`).map(e=>e.trim()).filter(e=>e),t=document.getElementById(`preview-tags-container`);if(t&&(t.replaceChildren(),e.length>0)){let n=document.createElement(`span`);n.textContent=`•`;let r=document.createElement(`span`);r.textContent=`Sentimentos: ${e.join(`, `)}`,t.append(n,` `,r)}},250)),m.addEventListener(`change`,()=>{h.style.display=m.value===`scheduled`?`block`:`none`});let g=document.getElementById(`poem-audio-file`),_=document.getElementById(`poem-audio-url`),v=document.getElementById(`audio-upload-status`),y=document.getElementById(`audio-preview-container`),b=document.getElementById(`audio-preview-player`),x=document.getElementById(`audio-file-name`),S=document.getElementById(`remove-audio-btn`);g&&g.addEventListener(`change`,async e=>{let t=e.target.files?.[0];if(!t)return;let i=[`.mp3`,`.wav`,`.m4a`,`.aac`,`.ogg`],a=`.`+t.name.split(`.`).pop().toLowerCase();if(!i.includes(a)&&!t.type.startsWith(`audio/`)){alert(`Formato de áudio inválido. Por favor selecione um arquivo .mp3, .wav, .m4a ou .aac.`),g.value=``;return}let o={".m4a":`audio/mp4`,".mp3":`audio/mpeg`,".wav":`audio/wav`,".aac":`audio/aac`,".ogg":`audio/ogg`}[a]||t.type||`audio/mpeg`;if(t.size>10485760){alert(`O arquivo de áudio excede o tamanho máximo de 10MB.`),g.value=``;return}v.textContent=`Enviando áudio...`,v.style.color=`var(--accent-subtle)`,g.disabled=!0;try{let e=`narration_${Date.now()}${a}`,i=await n.storage.from(`audios`).upload(e,t,{contentType:o,upsert:!0}),s=`audios`;if(i.error&&(console.warn(`Bucket audios retornou erro, tentando fallback para avatars:`,i.error),i=await n.storage.from(`avatars`).upload(e,t,{contentType:o,upsert:!0}),s=`avatars`),i.error)throw i.error;let{data:c}=n.storage.from(s).getPublicUrl(e),l=c.publicUrl;_.value=l,b.src=r(l),b.load(),x.textContent=t.name,y.style.display=`flex`,S.style.display=`inline-block`,v.textContent=`Áudio carregado!`,v.style.color=`var(--success)`}catch(e){console.error(`Erro no upload de áudio:`,e),v.textContent=`Erro no envio`,v.style.color=`var(--error)`,alert(`Erro ao enviar áudio: `+(e.message||`Falha de conexão`))}finally{g.disabled=!1,g.value=``,setTimeout(()=>{v&&(v.textContent=``)},3500)}}),S&&S.addEventListener(`click`,()=>{confirm(`Deseja realmente remover o áudio desta poesia?`)&&(_.value=``,b.pause(),b.removeAttribute(`src`),b.load(),y.style.display=`none`,S.style.display=`none`,x.textContent=``,v.textContent=`Áudio removido.`,v.style.color=`var(--text-muted)`,setTimeout(()=>{v&&(v.textContent=``)},2500))});let C=()=>{let e=p.value.split(`,`).map(e=>e.trim()).filter(e=>e),t=document.getElementById(`scheduled-at`);return{title:document.getElementById(`poem-title`).value,slug:document.getElementById(`poem-slug`).value,content:document.getElementById(`poem-content-input`).value,excerpt:document.getElementById(`poem-excerpt`).value,tags:e,audio_url:_&&_.value.trim()||null,status:document.getElementById(`poem-status`).value,scheduled_at:t.value?new Date(t.value).toISOString():null}};document.getElementById(`editor-form`).addEventListener(`submit`,async e=>{e.preventDefault();let r=document.getElementById(`save-btn`);r.innerText=`Salvando...`,r.disabled=!0;let i=C();if(i.status===`scheduled`&&!i.scheduled_at){alert(`Por favor, defina uma data para o agendamento.`),r.innerText=`Gravar Alterações`,r.disabled=!1;return}i.status===`published`&&c.status!==`published`&&(i.published_at=new Date().toISOString());let a=null;if(a=s?(await n.from(`poems`).update(i).eq(`id`,s)).error:(await n.from(`poems`).insert([i])).error,a){console.error(a),alert(`Erro ao salvar: `+a.message),r.innerText=`Gravar Alterações`,r.disabled=!1;return}t(`/admin`)});let w=document.getElementById(`publish-btn`);if(w){let e=!1;w.addEventListener(`click`,async r=>{if(r.preventDefault(),!document.getElementById(`editor-form`).reportValidity())return;if(!e){w.innerText=`Tem certeza? Clique para confirmar.`,w.style.background=`var(--error)`,e=!0,setTimeout(()=>{w&&!w.disabled&&(w.innerText=`Publicar e Notificar Assinantes`,w.style.background=`var(--success)`,e=!1)},4e3);return}w.innerText=`Publicando...`,w.disabled=!0;let i=C();i.status=`published`,i.published_at=new Date().toISOString();let a=s,o=null;if(s)o=(await n.from(`poems`).update(i).eq(`id`,s)).error;else{let e=await n.from(`poems`).insert([i]).select().single();o=e.error,e.data&&(a=e.data.id)}if(o){console.error(o),alert(`Erro ao publicar: `+o.message),w.innerText=`Publicar e Notificar Assinantes`,w.disabled=!1;return}if(a)try{w.innerText=`Enviando newsletter...`;let{data:e,error:t}=await n.functions.invoke(`send-newsletter`,{body:{poemId:a}});if(t)throw t;alert(`Obra publicada e newsletter enviada com sucesso para ${e.count} assinantes!`)}catch(e){console.error(`Newsletter erro:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Obra publicada, mas houve um erro ao enviar a newsletter:\n${t||e.message||`Erro na Edge Function`}`)}t(`/admin`)})}},async renderEmailHistory(e){e.innerHTML=`<div class="loading">Carregando histórico e dados...</div>`;try{let[t,r]=await Promise.all([n.from(`email_campaign_logs`).select(`id, sent_at, status, details, poem_id, poems(title)`).order(`sent_at`,{ascending:!1}),n.from(`poems`).select(`id, title`).eq(`status`,`published`).order(`created_at`,{ascending:!1})]);if(t.error)throw t.error;if(r.error)throw r.error;let i=t.data||[],s=r.data||[],c=i.length,l=i.filter(e=>e.status===`success`).length,u=c>0?(l/c*100).toFixed(1):`100.0`,d=i[0]||null;e.innerHTML=`
        <div style="font-family: var(--font-ui); display: grid; gap: var(--space-md);">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: var(--space-md); margin-bottom: var(--space-md);">
            ...
            <div class="kpi-card" style="background: var(--bg-elevated); padding: var(--space-lg); border-radius: 6px; border: 1px solid var(--border-subtle); display: flex; flex-direction: column; justify-content: space-between;">
              ...
              <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: var(--space-3xs);">
                ${d?(e=>{let t=new Date(e),n=new Date-t,r=Math.floor(n/6e4),i=Math.floor(n/36e5),a=Math.floor(n/864e5);return r<1?`Agora mesmo`:r<60?`Há ${r} min`:i<24?`Há ${i} h`:a===1?`Ontem`:a<30?`Há ${a} dias`:t.toLocaleDateString(`pt-BR`)})(d.sent_at):`Nenhum dado`}
              </div>
            </div>
          </div>
          ...
          <div style="font-size: 0.85rem; color: var(--text-secondary); display: flex; justify-content: space-between; align-items: center; padding: 0 4px;">
            <div>
              Mostrando <strong id="results-count" style="color: var(--text-primary);">0</strong> de <strong>${i.length}</strong> envios
            </div>
          </div>
            ...
            <form id="manual-dispatch-form" style="display: flex; flex-direction: column; gap: var(--space-md); margin-top: var(--space-2xs);">
              <div style="display: flex; flex-direction: column; gap: var(--space-3xs);">
                ...
                <select id="dispatch-poem-select" required style="padding: var(--space-xs); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 4px; font-size: 0.95rem; cursor: pointer; width: 100%;">
                  ...
                  ${s.map(e=>`<option value="${e.id}">${a(e.title)}</option>`).join(``)}
                </select>
      ...
      `;let f=e.querySelector(`#email-tbody`),p=e.querySelector(`#results-count`),m=e.querySelector(`#manual-dispatch-modal`),h=e.querySelector(`#log-details-modal`);e.querySelector(`#open-dispatch-modal-btn`).addEventListener(`click`,()=>{m.style.display=`flex`,e.querySelector(`#manual-dispatch-form`).reset()}),e.querySelector(`#close-dispatch-modal-btn`).addEventListener(`click`,()=>{m.style.display=`none`}),e.querySelector(`#close-details-modal-btn`).addEventListener(`click`,()=>{h.style.display=`none`}),[m,h].forEach(e=>{e.addEventListener(`click`,t=>{t.target===e&&(e.style.display=`none`)})});let g=null,_=async(t,r,i)=>{if(!t){alert(`Não é possível reenviar: id da obra indisponível.`);return}if(!confirm(`Deseja reenviar a newsletter da obra "${r}" para todos os assinantes ativos?`))return;let a=i.innerText;i.disabled=!0,i.innerText=`Enviando...`,i.style.opacity=`0.5`;try{let{data:i,error:a}=await n.functions.invoke(`send-newsletter`,{body:{poemId:t}});if(a)throw a;alert(`Newsletter para "${r}" reenviada com sucesso para ${i?.count||0} assinantes!`),h.style.display=`none`,this.renderEmailHistory(e)}catch(e){console.error(`Erro ao reenviar:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Erro ao enviar newsletter:\n${t||e.message||`Erro na Edge Function`}`),i.disabled=!1,i.innerText=a,i.style.opacity=`1`}};e.querySelector(`#resend-from-modal-btn`).addEventListener(`click`,async e=>{e.preventDefault(),g&&g.poem_id&&await _(g.poem_id,g.poems?.title||`Desconhecido`,e.currentTarget)});let v=t=>{if(p.innerText=t.length,t.length===0){f.innerHTML=`
            <tr>
              <td colspan="5" style="padding: var(--space-xl) 0; text-align: center; color: var(--text-muted); font-style: italic;">
                Nenhum registro de envio encontrado com os filtros aplicados.
              </td>
            </tr>
          `;return}f.innerHTML=t.map(e=>{let t=e.poems?.title||`Desconhecido`,n=new Date(e.sent_at).toLocaleString(`pt-BR`,{dateStyle:`short`,timeStyle:`short`}),r=e.status===`success`?`
            ...
            `,i=e.details?e.details.length>50?`${a(e.details.slice(0,48))}...`:a(e.details):`-`;return`
            <tr style="border-bottom: 1px solid var(--border-subtle); transition: background-color var(--transition-fast);">
              <td style="padding: var(--space-md) 0; font-family: var(--font-display); font-size: 1.15rem; color: var(--text-primary); font-weight: 400;">${a(t)}</td>
              ...
              <td style="padding: var(--space-md) 0; text-align: right; font-family: var(--font-ui);">
                ...
                <button class="resend-log-btn" data-poem-id="${e.poem_id||``}" data-title="${a(t)}" style="color: var(--accent-subtle); background: transparent; border: none; cursor: pointer; font-size: 0.85rem; transition: opacity var(--transition-fast); text-decoration: none; padding: 0; font-weight: 500;">Reenviar</button>
              </td>
            </tr>
          `}).join(``),f.querySelectorAll(`tr`).forEach(e=>{e.style.transition=`background-color var(--transition-fast)`,e.addEventListener(`mouseenter`,()=>{e.style.backgroundColor=`var(--bg-secondary)`}),e.addEventListener(`mouseleave`,()=>{e.style.backgroundColor=`transparent`})}),f.querySelectorAll(`.view-details-btn`).forEach(n=>{n.addEventListener(`click`,()=>{let r=n.dataset.id,i=t.find(e=>e.id===r);if(!i)return;g=i,e.querySelector(`#detail-poem-title`).innerText=i.poems?.title||`Desconhecido`,e.querySelector(`#detail-date`).innerText=new Date(i.sent_at).toLocaleString(`pt-BR`,{dateStyle:`long`,timeStyle:`medium`});let a=i.status===`success`;e.querySelector(`#detail-status-badge`).innerHTML=a?`<span style="padding: 0.2rem 0.6rem; border-radius: 20px; font-family: var(--font-ui); font-size: 0.7rem; font-weight: 600; border: 1px solid rgba(58, 140, 84, 0.3); background: rgba(58, 140, 84, 0.08); color: var(--success); text-transform: uppercase;">Sucesso</span>`:`<span style="padding: 0.2rem 0.6rem; border-radius: 20px; font-family: var(--font-ui); font-size: 0.7rem; font-weight: 600; border: 1px solid rgba(204, 74, 74, 0.3); background: rgba(204, 74, 74, 0.08); color: var(--error); text-transform: uppercase;">Falha</span>`,e.querySelector(`#detail-description`).innerText=i.details||`Nenhum detalhe adicional disponível.`;let o=e.querySelector(`#resend-from-modal-btn`);i.poem_id?(o.style.display=`inline-block`,o.innerText=a?`Disparar Novamente`:`Tentar Reenviar`):o.style.display=`none`,h.style.display=`flex`})}),f.querySelectorAll(`.resend-log-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.poemId,r=e.dataset.title;_(n,r,e)})})},y=e.querySelector(`#email-search`),b=e.querySelector(`#email-filter-status`),x=e.querySelector(`#email-sort`),S=()=>{let e=y.value.toLowerCase().trim(),t=b.value,n=x.value,r=[...i];e&&(r=r.filter(t=>{let n=(t.poems?.title||``).toLowerCase(),r=(t.details||``).toLowerCase();return n.includes(e)||r.includes(e)})),t!==`all`&&(r=t===`success`?r.filter(e=>e.status===`success`):r.filter(e=>e.status!==`success`)),n===`newest`?r.sort((e,t)=>new Date(t.sent_at)-new Date(e.sent_at)):n===`oldest`?r.sort((e,t)=>new Date(e.sent_at)-new Date(t.sent_at)):n===`title-az`?r.sort((e,t)=>{let n=e.poems?.title||``,r=t.poems?.title||``;return n.localeCompare(r)}):n===`title-za`&&r.sort((e,t)=>{let n=e.poems?.title||``;return(t.poems?.title||``).localeCompare(n)}),v(r)};y.addEventListener(`input`,o(S,150)),b.addEventListener(`change`,S),x.addEventListener(`change`,S),S(),e.querySelector(`#manual-dispatch-form`).addEventListener(`submit`,async t=>{t.preventDefault();let r=e.querySelector(`#dispatch-poem-select`),i=r.value,a=r.options[r.selectedIndex].text,o=e.querySelector(`#dispatch-target-email`),s=o?o.value.trim():``,c=e.querySelector(`#dispatch-confirm-chk`);if(!s&&!c.checked){alert(`Você precisa confirmar o envio para todos os assinantes caso não preencha um e-mail de destino.`);return}let l=e.querySelector(`#submit-dispatch-btn`),u=e.querySelector(`#close-dispatch-modal-btn`);l.disabled=!0,l.innerText=`Enviando...`,r.disabled=!0,o&&(o.disabled=!0),c.disabled=!0,u.style.display=`none`;try{let t={poemId:i};s&&(t.targetEmail=s);let{data:r,error:o}=await n.functions.invoke(`send-newsletter`,{body:t});if(o)throw o;alert(s?`Newsletter para "${a}" enviada com sucesso para o e-mail: ${s}!`:`Newsletter para "${a}" enviada com sucesso para ${r?.count||0} assinantes!`),r?.logError&&(console.error(`Log error:`,r.logError),alert(`ATENÇÃO: Os emails foram enviados, mas houve um erro ao salvar o histórico no banco de dados.\nErro: ${r.logError.message||JSON.stringify(r.logError)}`)),m.style.display=`none`,this.renderEmailHistory(e)}catch(e){console.error(`Erro na Edge Function:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Erro ao disparar newsletter:\n${t||e.message||`Erro inesperado`}`),l.disabled=!1,l.innerText=`Disparar Newsletter`,r.disabled=!1,o&&(o.disabled=!1),c.disabled=!1,u.style.display=`inline-block`}})}catch(t){console.error(t),e.innerHTML=`<div class="error">Erro ao carregar dados do histórico: ${t.message}</div>`}},async renderSubscribers(e){e.innerHTML=`<div class="loading">Carregando assinantes...</div>`;let{data:t,error:r}=await n.from(`subscribers`).select(`email, active, created_at, unsubscribed_at`).order(`created_at`,{ascending:!1});if(r){e.innerHTML=`<div class="error">Erro ao carregar: ${r.message}</div>`;return}if(!t||t.length===0){e.innerHTML=`<p>Nenhum assinante encontrado.</p>`;return}let i=t.filter(e=>e.active).length,a=t.length,o=new Date;o.setDate(o.getDate()-30);let s=t.filter(e=>new Date(e.created_at)>o).length,c=t.filter(e=>!e.active&&e.unsubscribed_at&&new Date(e.unsubscribed_at)>o).length;e.innerHTML=`
...
`),n=encodeURI(e),r=document.createElement(`a`);r.setAttribute(`href`,n),r.setAttribute(`download`,`assinantes_${new Date().toISOString().split(`T`)[0]}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)})},async renderComments(e){e.innerHTML=`<div class="loading">Carregando comentários...</div>`;let{data:t,error:r}=await n.from(`poem_comments`).select(`id, author_name, content, approved, created_at, poems(title)`).order(`created_at`,{ascending:!1});if(r){e.innerHTML=`<div class="error">Erro ao carregar: ${r.message}</div>`;return}if(!t||t.length===0){e.innerHTML=`<p>Nenhum comentário encontrado.</p>`;return}e.innerHTML=`
      ...
      <tr style="border-bottom: 1px solid var(--border-subtle); transition: background-color var(--transition-fast);">
        ...
        <td style="padding: var(--space-md) 0;">
          <div style="font-family: var(--font-display); font-size: 1.1rem;">${a(e.author_name)}</div>
          ...
          <div style="font-family: var(--font-body); line-height: 1.4; color: var(--text-primary); max-width: 500px;">${a(e.content)}</div>
        </td>
    ...
    `,e.querySelectorAll(`.approve-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{let r=t.dataset.id,{error:i}=await n.from(`poem_comments`).update({approved:!0}).eq(`id`,r);i?alert(`Erro ao aprovar: `+i.message):this.renderComments(e)})}),e.querySelectorAll(`.delete-comment-btn`).forEach(t=>{t.addEventListener(`click`,async()=>{if(!confirm(`Excluir este comentário?`))return;let r=t.dataset.id,{error:i}=await n.from(`poem_comments`).delete().eq(`id`,r);i?alert(`Erro ao excluir: `+i.message):this.renderComments(e)})})}};export{s as default};