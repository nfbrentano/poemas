        ...
        <div class="editor-layout">
          <div class="editor-pane">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--space-lg);">
              <div>
                ...
                <input type="text" id="poem-title" value="${a(s.title)}" required style="width: 100%; font-size: 1.5rem; font-family: var(--font-display); padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0;">
              </div>
              <div>
                ...
                <input type="text" id="poem-slug" value="${a(s.slug)}" required style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0; color: var(--text-muted);">
              </div>
            </div>
            ...
            <div style="margin-top: var(--space-md);">
              ...
              <textarea id="poem-content-input" required style="width: 100%; min-height: 500px; font-family: var(--font-body); font-size: 1.1rem; line-height: 1.6; padding: var(--space-md); border: 1px solid var(--border-strong); background: var(--bg-primary); border-radius: 2px; color: var(--text-primary); resize: vertical;">${a(s.content)}</textarea>
            </div>
            ...
            <div>
              ...
              <textarea id="poem-excerpt" style="width: 100%; min-height: 80px; font-family: var(--font-body); font-size: 1rem; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); border-radius: 2px; resize: vertical;">${a(s.excerpt||``)}</textarea>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-lg);">
              <div>
                ...
                <input type="text" id="poem-tags" value="${a(s.tags?s.tags.join(`, `):``)}" placeholder="Ex: Amor, Saudade, Melancolia" style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0;">
              </div>
              <div>
                ...
                <select id="poem-status" style="width: 100%; padding: var(--space-xs) 0; border: none; border-bottom: 1px solid var(--border-strong); background: transparent; border-radius: 0; color: var(--text-primary);">
                  <option value="draft" ${s.status===`draft`?`selected`:``}>Rascunho</option>
                  <option value="scheduled" ${s.status===`scheduled`?`selected`:``}>Agendado</option>
                  <option value="published" ${s.status===`published`?`selected`:``}>Publicado</option>
                </select>
              </div>
            </div>

            <div id="scheduling-fields" style="margin-top: var(--space-md); ${s.status===`scheduled`?``:`display: none;`}">
              ...
              <input type="datetime-local" id="scheduled-at" value="${s.scheduled_at?new Date(s.scheduled_at).toISOString().slice(0,16):``}" style="width: 100%; padding: var(--space-sm); border: 1px solid var(--border-strong); background: var(--bg-primary); color: var(--text-primary); border-radius: 2px;">
              <p class="field-help">Se definido e o status for "Agendado", o poema será publicado automaticamente.</p>
              ${s.status===`scheduled`?`<p class="field-help" style="color: var(--accent-subtle); font-style: italic;">Este poema será publicado automaticamente em ${new Date(s.scheduled_at).toLocaleString(`pt-BR`)}.</p>`:``}
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: var(--space-md); margin-top: var(--space-lg); border-top: 1px solid var(--border-subtle); padding-top: var(--space-lg);">
              ...
              ${s.status===`draft`?`<button type="button" class="btn-primary" id="publish-btn" style="padding: 0.75rem 1.5rem; background: var(--success); color: #fff; border-radius: 2px; font-weight: 500;">Publicar Agora</button>`:``}
            </div>
          </div>
          ...
          <div class="preview-pane">
            ...
            <article class="preview-poem">
              <h1 id="preview-title">${a(s.title||`Título da Obra`)}</h1>
              <div class="poem-meta preview-meta">
                <span id="preview-date">${s.published_at?new Date(s.published_at).toLocaleDateString(`pt-BR`):new Date().toLocaleDateString(`pt-BR`)}</span>
                <span id="preview-tags-container">${s.tags&&s.tags.length>0?`<span>•</span> <span>Sentimentos: ${a(s.tags.join(`, `))}</span>`:``}</span>
              </div>
              <div id="preview-content" class="poem-content">${a(s.content||``)}</div>
            </article>
          </div>
        </div>
      </form>
    `;let c=document.getElementById(`poem-title`),l=document.getElementById(`poem-slug`);c.addEventListener(`input`,()=>{if(document.getElementById(`preview-title`).textContent=c.value||`Título da Obra`,!r||l.value===``){let e=c.value.toLowerCase().trim().replace(/[áàãâä]/g,`a`).replace(/[éèêë]/g,`e`).replace(/[íìîï]/g,`i`).replace(/[óòõôö]/g,`o`).replace(/[úùûü]/g,`u`).replace(/ç/g,`c`).replace(/[^a-z0-9\s-]/g,``).replace(/\s+/g,`-`).replace(/-+/g,`-`).replace(/^-|-$/g,``);l.value=e}});let u=document.getElementById(`poem-content-input`),d=document.getElementById(`preview-content`),f=document.getElementById(`poem-tags`),p=document.getElementById(`poem-status`),m=document.getElementById(`scheduling-fields`);u.addEventListener(`input`,o(()=>{d.textContent=u.value},250)),f.addEventListener(`input`,o(()=>{let e=f.value.split(`,`).map(e=>e.trim()).filter(e=>e),t=document.getElementById(`preview-tags-container`);if(t&&(t.replaceChildren(),e.length>0)){let n=document.createElement(`span`);n.textContent=`•`;let r=document.createElement(`span`);r.textContent=`Sentimentos: ${e.join(`, `)}`,t.append(n,` `,r)}},250)),p.addEventListener(`change`,()=>{m.style.display=p.value===`scheduled`?`block`:`none`});let h=()=>{let e=f.value.split(`,`).map(e=>e.trim()).filter(e=>e),t=document.getElementById(`scheduled-at`);return{title:document.getElementById(`poem-title`).value,slug:document.getElementById(`poem-slug`).value,content:document.getElementById(`poem-content-input`).value,excerpt:document.getElementById(`poem-excerpt`).value,tags:e,status:document.getElementById(`poem-status`).value,scheduled_at:t.value?new Date(t.value).toISOString():null}};document.getElementById(`editor-form`).addEventListener(`submit`,async e=>{e.preventDefault();let i=document.getElementById(`save-btn`);i.innerText=`Salvando...`,i.disabled=!0;let a=h();if(a.status===`scheduled`&&!a.scheduled_at){alert(`Por favor, defina uma data para o agendamento.`),i.innerText=`Gravar Alterações`,i.disabled=!1;return}a.status===`published`&&s.status!==`published`&&(a.published_at=new Date().toISOString());let o=null;if(o=r?(await n.from(`poems`).update(a).eq(`id`,r)).error:(await n.from(`poems`).insert([a])).error,o){console.error(o),alert(`Erro ao salvar: `+o.message),i.innerText=`Gravar Alterações`,i.disabled=!1;return}t(`/admin`)});let g=document.getElementById(`publish-btn`);if(g){let e=!1;g.addEventListener(`click`,async i=>{if(i.preventDefault(),!document.getElementById(`editor-form`).reportValidity())return;if(!e){g.innerText=`Tem certeza? Clique para confirmar.`,g.style.background=`var(--error)`,e=!0,setTimeout(()=>{g&&!g.disabled&&(g.innerText=`Publicar e Notificar Assinantes`,g.style.background=`var(--success)`,e=!1)},4e3);return}g.innerText=`Publicando...`,g.disabled=!0;let a=h();a.status=`published`,a.published_at=new Date().toISOString();let o=r,s=null;if(r)s=(await n.from(`poems`).update(a).eq(`id`,r)).error;else{let e=await n.from(`poems`).insert([a]).select().single();s=e.error,e.data&&(o=e.data.id)}if(s){console.error(s),alert(`Erro ao publicar: `+s.message),g.innerText=`Publicar e Notificar Assinantes`,g.disabled=!1;return}if(o)try{g.innerText=`Enviando newsletter...`;let{data:e,error:t}=await n.functions.invoke(`send-newsletter`,{body:{poemId:o}});if(t)throw t;alert(`Obra publicada e newsletter enviada com sucesso para ${e.count} assinantes!`)}catch(e){console.error(`Newsletter erro:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Obra publicada, mas houve um erro ao enviar a newsletter:\n${t||e.message||`Erro na Edge Function`}`)}t(`/admin`)})}},async renderEmailHistory(e){e.innerHTML=`<div class="loading">Carregando histórico e dados...</div>`;try{let[t,r]=await Promise.all([n.from(`email_campaign_logs`).select(`id, sent_at, status, details, poem_id, poems(title)`).order(`sent_at`,{ascending:!1}),n.from(`poems`).select(`id, title`).eq(`status`,`published`).order(`created_at`,{ascending:!1})]);if(t.error)throw t.error;if(r.error)throw r.error;let i=t.data||[],s=r.data||[],c=i.length,l=i.filter(e=>e.status===`success`).length,u=c>0?(l/c*100).toFixed(1):`100.0`,d=i[0]||null;e.innerHTML=`
        <div style="font-family: var(--font-ui); display: grid; gap: var(--space-md);">
