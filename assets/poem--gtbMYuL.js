...
import{a as e,i as t,n,r,s as i,t as a}from"./index-BWGE2Wiv.js";import{r as o,t as s}from"./html-Ca31mHym.js";import{t as ee}from"./seo-BlITGSmq.js";import{t as te}from"./newsletter-B0DNuzGh.js";var c=[`🕯️`,`💧`,`🌿`,`🌙`,`✨`];function l(){let e=localStorage.getItem(`reaction_session_id`);if(!e){if(typeof crypto<`u`&&typeof crypto.randomUUID==`function`)e=crypto.randomUUID();else if(typeof crypto<`u`&&typeof crypto.getRandomValues==`function`){let t=new Uint8Array(16);crypto.getRandomValues(t),t[6]=t[6]&15|64,t[8]=t[8]&63|128;let n=Array.from(t,e=>e.toString(16).padStart(2,`0`)).join(``);e=`${n.slice(0,8)}-${n.slice(8,12)}-${n.slice(12,16)}-${n.slice(16,20)}-${n.slice(20)}`}e&&localStorage.setItem(`reaction_session_id`,e)}return e}async function ne(e){let{data:t,error:n}=await i.from(`poem_reactions`).select(`emoji, session_id`).eq(`poem_id`,e);if(n)return{counts:{},userReactions:new Set};let r=l(),a={},o=new Set;return c.forEach(e=>a[e]=0),(t||[]).forEach(e=>{a[e.emoji]=(a[e.emoji]||0)+1,e.session_id===r&&o.add(e.emoji)}),{counts:a,userReactions:o}}async function re(e,t){let n=l(),{data:r}=await i.from(`poem_reactions`).select(`id`).eq(`poem_id`,e).eq(`session_id`,n).eq(`emoji`,t).maybeSingle();return r?(await i.from(`poem_reactions`).delete().eq(`id`,r.id),`removed`):(await i.from(`poem_reactions`).insert({poem_id:e,emoji:t,session_id:n}),`added`)}var u={init(){if(!document.getElementById(`toast-container`)){let e=document.createElement(`div`);e.id=`toast-container`,e.className=`toast-container`,e.setAttribute(`aria-live`,`polite`),document.body.appendChild(e)}},show(e,t=`default`,n=3e3){this.init();let r=document.getElementById(`toast-container`),i=document.createElement(`div`);i.className=`toast-message toast-${t}`;let a=``;t===`success`&&(a=`✓ `),t===`error`&&(a=`⚠ `),t===`info`&&(a=`ℹ `),t===`heart`&&(a=`♥ `),i.innerHTML=`<span class="toast-icon">${a}</span><span class="toast-text">${e}</span>`,r.appendChild(i),i.offsetWidth,i.classList.add(`show`),setTimeout(()=>{i.classList.remove(`show`),i.addEventListener(`transitionend`,()=>{i.parentNode&&i.parentNode.removeChild(i)})},n)}},ie=`poemas_db`,d=`poem_notes`,f=2,p=null;function m(){return p?Promise.resolve(p):new Promise((e,t)=>{let n=indexedDB.open(ie,f);n.onsuccess=t=>{p=t.target.result,e(p)},n.onerror=e=>t(e.target.error)})}var h={async get(e){let t=await m();return new Promise((n,r)=>{let i=t.transaction(d,`readonly`).objectStore(d).get(e);i.onsuccess=()=>n(i.result?i.result.note:``),i.onerror=()=>r(i.error)})},async save(e,t){let n=await m();return new Promise((r,i)=>{let a=n.transaction(d,`readwrite`).objectStore(d),o={slug:e,note:t,updated_at:new Date().toISOString()},s=a.put(o);s.onsuccess=()=>r(!0),s.onerror=()=>i(s.error)})},async delete(e){let t=await m();return new Promise((n,r)=>{let i=t.transaction(d,`readwrite`).objectStore(d).delete(e);i.onsuccess=()=>n(!0),i.onerror=()=>r(i.error)})}};function ae(e,t){let n;return function(){let r=arguments,i=this;n||(e.apply(i,r),n=!0,setTimeout(()=>n=!1,t))}}var g=null,_=null,v=null,y=null,b=!1,x=``,S={meta:{title:`Poema`},cleanup(){g&&window.removeEventListener(`scroll`,g),_&&document.removeEventListener(`touchstart`,_),v&&document.removeEventListener(`touchend`,v),y&&document.removeEventListener(`keydown`,y),document.documentElement.classList.remove(`immersive-mode`),g=null,_=null,v=null,y=null,b=!1,x=``},async render(l,ie){let d=ie.slug;l.innerHTML=`
    ...
    `,console.log(`[Poem] Fetching slug:`,d);let{data:f,error:p}=await i.rpc(`get_poem_with_navigation`,{target_slug:d});p&&console.error(`[Poem] RPC Error:`,p);let m=f&&Array.isArray(f)&&f.length>0?f[0]:null;if(p||!m){console.warn(`[Poem] Poem not found or error occurred`),l.innerHTML=`
      ...
      `;return}let S=m.prev_slug,C=m.next_slug,oe=m.prev_title,w=m.next_title;try{n.add(m),window.dispatchEvent(new CustomEvent(`history-updated`))}catch(e){console.error(`Erro ao salvar no histórico:`,e)}let se=o(m.content||``).replace(/\s+/g,` `).trim().split(` `).filter(e=>e.length>0).length,ce=Math.ceil(se/200),le=ce<=1?`1 min de leitura`:`${ce} min de leitura`;e(`/poema/`+m.slug,m.id);let ue=window.location.href,de=o(m.excerpt||m.content||``).replace(/\s+/g,` `).trim().slice(0,160)+`...`,fe=`${window.location.origin}/poemas/og-cover.jpg`;ee({title:m.title,description:de,url:ue,imageUrl:fe,type:`article`,publishedTime:m.published_at,tags:m.tags});let{data:{session:pe}}=await i.auth.getSession(),me=!!pe,he=(e=>{if(!e)return``;let t=e.split(/\n\s*\n/).filter(e=>e.trim()),n=0;return t.map(e=>`<div class="stanza stagger-reveal">${e.split(`
`).map(e=>(n++,`<span class="line-reveal" style="transition-delay: ${n*.05}s">${e}</span>`)).join(``)}</div>`).join(``)})(m.content);l.innerHTML=`
        ...
        <article class="single-poem fade-in">
          <header>
            ...
            <div class="poem-meta">
              ...
              <span>•</span>
              <span class="reading-time">${le}</span>
            </div>
          </header>

          
          
          <div id="poem-text" class="poem-content">${he}</div>
    ...
    `,(()=>{if(window.matchMedia(`(prefers-reduced-motion: reduce)`).matches){l.querySelectorAll(`.stagger-reveal`).forEach(e=>e.classList.add(`revealed`));return}let e=new IntersectionObserver((e,t)=>{e.forEach(e=>{e.isIntersecting&&(e.target.classList.add(`revealed`),t.unobserve(e.target))})},{root:null,rootMargin:`0px 0px -10% 0px`,threshold:.1});l.querySelectorAll(`.stagger-reveal`).forEach(t=>e.observe(t))})();let T=window.location.href,E=`Leia "${m.title}", um poema de Natanael Brentano:`;document.querySelectorAll(`.share-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.platform,n=``;t===`whatsapp`&&(n=`https://api.whatsapp.com/send?text=${encodeURIComponent(E+` `+T)}`),t===`twitter`&&(n=`https://twitter.com/intent/tweet?text=${encodeURIComponent(E)}&url=${encodeURIComponent(T)}`),t===`facebook`&&(n=`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(T)}`),window.open(n,`_blank`,`noopener,noreferrer`)})});let D=document.getElementById(`web-share-btn`);navigator.share?D.addEventListener(`click`,async()=>{try{await navigator.share({title:m.title,text:E,url:T})}catch{}}):(D.innerText=`Copiar Link`,D.addEventListener(`click`,()=>{navigator.clipboard.writeText(T).then(()=>{u.show(`Link copiado para a área de transferência!`,`success`)})}));let{counts:ge,userReactions:_e}=await ne(m.id),ve=(e,t)=>{c.forEach(n=>{let r=l.querySelector(`.reaction-btn[data-emoji="${n}"]`),i=l.querySelector(`.reaction-count[data-count="${n}"]`);r&&r.classList.toggle(`reacted`,t.has(n)),i&&(i.textContent=e[n]||0)})};ve(ge,_e),l.querySelectorAll(`.reaction-btn`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.emoji;e.disabled=!0,await re(m.id,t);let{counts:n,userReactions:r}=await ne(m.id);ve(n,r),e.classList.add(`reacted`),setTimeout(()=>{e.style.animation=`none`,e.offsetHeight,e.style.animation=null},10),e.disabled=!1})}),(async()=>{let{data:e,error:t}=await i.from(`poem_comments`).select(`author_name, content, created_at`).eq(`poem_id`,m.id).eq(`approved`,!0).order(`created_at`,{ascending:!0}),n=document.getElementById(`comments-list`);if(t||!e||e.length===0){n.innerHTML=`<p class="comments-empty">Silêncio... nenhum comentário ainda.</p>`;return}n.innerHTML=e.map(e=>`
      ...
      `).join(``)})();let ye=document.getElementById(`comment-form`);ye?.addEventListener(`submit`,async e=>{e.preventDefault();let t=document.getElementById(`comment-author`).value,n=document.getElementById(`comment-content`).value,r=document.getElementById(`submit-comment-btn`);document.getElementById(`comment-msg`),r.disabled=!0,r.innerText=`Enviando...`;let{error:a}=await i.from(`poem_comments`).insert([{poem_id:m.id,author_name:t,content:n}]);a?(u.show(`Erro ao enviar comentário.`,`error`),r.disabled=!1,r.innerText=`Enviar Nota`):(u.show(`Sua nota foi enviada e aguarda moderação.`,`success`),ye.reset(),r.disabled=!1,r.innerText=`Enviar Nota`)});let O=document.getElementById(`fav-btn`),be=async()=>{let e=await a.has(m.slug);O&&(O.querySelector(`.fav-icon`).textContent=e?`♥`:`♡`,O.querySelector(`.fav-text`).textContent=e?`Salvo`:`Salvar`,O.classList.toggle(`active`,e))};be(),O?.addEventListener(`click`,async()=>{await a.has(m.slug)?(await a.remove(m.slug),u.show(`Obra removida dos itens salvos.`,`info`)):(await a.save(m),u.show(`Obra salva com sucesso.`,`heart`),`serviceWorker`in navigator&&navigator.serviceWorker.controller&&fetch(`/poemas/poema/${m.slug}`).catch(()=>{})),be(),O.classList.remove(`animate-fav`),O.offsetWidth,O.classList.add(`animate-fav`),setTimeout(()=>O.classList.remove(`animate-fav`),800),window.dispatchEvent(new CustomEvent(`favorites-updated`))});let xe=document.getElementById(`immersive-btn`),Se=document.getElementById(`immersive-exit-btn`),k=document.getElementById(`immersive-size-slider`),A=document.getElementById(`immersive-size-value`),j=document.getElementById(`immersive-height-slider`),M=document.getElementById(`immersive-height-value`),N=document.getElementById(`poem-text`),P=!1,F=localStorage.getItem(`immersive-reading-font-size`)||`20`,I=localStorage.getItem(`immersive-reading-line-height`)||`22`;N&&(N.style.setProperty(`--immersive-font-size`,`${F}px`),N.style.setProperty(`--immersive-line-height`,`${parseFloat(I)/10}`)),k&&A&&(k.value=F,A.textContent=`${F}px`,k.addEventListener(`input`,e=>{let t=e.target.value;A.textContent=`${t}px`,N?.style.setProperty(`--immersive-font-size`,`${t}px`),localStorage.setItem(`immersive-reading-font-size`,t)})),j&&M&&(j.value=I,M.textContent=`${(parseFloat(I)/10).toFixed(1)}`,j.addEventListener(`input`,e=>{let t=e.target.value,n=(parseFloat(t)/10).toFixed(1);M.textContent=n,N?.style.setProperty(`--immersive-line-height`,n),localStorage.setItem(`immersive-reading-line-height`,t)}));let Ce=()=>{if(P=!0,document.documentElement.classList.add(`immersive-mode`),!localStorage.getItem(`immersive-hint-shown`)){let e=document.getElementById(`immersive-hint`);e&&(e.classList.add(`visible`),setTimeout(()=>{e.classList.remove(`visible`),localStorage.setItem(`immersive-hint-shown`,`true`)},3e3))}},L=()=>{P=!1,document.documentElement.classList.remove(`immersive-mode`)};xe?.addEventListener(`click`,Ce),Se?.addEventListener(`click`,L);let R=document.getElementById(`note-btn`),z=document.getElementById(`notes-panel`),B=document.getElementById(`note-textarea`),we=document.getElementById(`save-note-btn`),Te=document.getElementById(`delete-note-btn`),V=async()=>{try{let e=await h.get(d);B&&(B.value=e||``);let t=!!e;z&&z.classList.toggle(`has-content`,t),R&&R.classList.toggle(`active`,t)}catch(e){console.error(`Erro ao ler notas:`,e)}};V(),R?.addEventListener(`click`,()=>{if(z){let e=z.style.display===`none`;z.style.display=e?`block`:`none`,e&&B&&B.focus()}}),we?.addEventListener(`click`,async()=>{let e=B.value.trim();if(!e){u.show(`Escreva algo para salvar ou use Excluir.`,`info`);return}try{await h.save(d,e),u.show(`Sua percepção foi salva offline.`,`success`),V()}catch{u.show(`Erro ao salvar nota.`,`error`)}}),Te?.addEventListener(`click`,async()=>{if(confirm(`Deseja excluir sua anotação pessoal?`))try{await h.delete(d),B&&(B.value=``),u.show(`Anotação excluída.`,`info`),V(),z&&(z.style.display=`none`)}catch{u.show(`Erro ao excluir nota.`,`error`)}});let H=l.querySelectorAll(`.size-btn`),U=l.querySelectorAll(`.family-btn`),W=l.querySelectorAll(`.height-btn`),G=l.querySelectorAll(`.align-btn`),K=(e,t)=>{e.forEach(e=>e.classList.toggle(`active`,e.dataset.size===t||e.dataset.family===t||e.dataset.height===t||e.dataset.align===t))},Ee=localStorage.getItem(`reading-font-size`)||`md`,De=localStorage.getItem(`reading-font-family`)||`serif`,Oe=localStorage.getItem(`reading-line-height`)||`normal`,ke=localStorage.getItem(`reading-alignment`)||`center`;document.documentElement.classList.remove(`font-reading-sm`,`font-reading-md`,`font-reading-lg`),document.documentElement.classList.add(`font-reading-${Ee}`),document.documentElement.classList.remove(`font-family-serif`,`font-family-sans`,`font-family-hand`),document.documentElement.classList.add(`font-family-${De}`),document.documentElement.classList.remove(`line-height-normal`,`line-height-relaxed`),document.documentElement.classList.add(`line-height-${Oe}`),document.documentElement.classList.remove(`align-reading-left`,`align-reading-center`),document.documentElement.classList.add(`align-reading-${ke}`),K(H,Ee),K(U,De),K(W,Oe),K(G,ke),H.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.size;document.documentElement.classList.remove(`font-reading-sm`,`font-reading-md`,`font-reading-lg`),document.documentElement.classList.add(`font-reading-${t}`),localStorage.setItem(`reading-font-size`,t),K(H,t)})}),U.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.family;document.documentElement.classList.remove(`font-family-serif`,`font-family-sans`,`font-family-hand`),document.documentElement.classList.add(`font-family-${t}`),localStorage.setItem(`reading-font-family`,t),K(U,t)})}),W.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.height;document.documentElement.classList.remove(`line-height-normal`,`line-height-relaxed`),document.documentElement.classList.add(`line-height-${t}`),localStorage.setItem(`reading-line-height`,t),K(W,t)})}),G.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.align;document.documentElement.classList.remove(`align-reading-left`,`align-reading-center`),document.documentElement.classList.add(`align-reading-${t}`),localStorage.setItem(`reading-alignment`,t),K(G,t)})});let Ae=l.querySelectorAll(`.ambient-btn`),q=document.getElementById(`ambient-audio`),je={rain:`https://cdn.pixabay.com/download/audio/2021/08/04/audio_0625c1539c.mp3?filename=heavy-rain-nature-sounds-8186.mp3`,fire:`https://cdn.pixabay.com/download/audio/2022/02/07/audio_27d8ce6605.mp3?filename=fireplace-sound-21271.mp3`};Ae.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sound;Ae.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),t===`silence`?q.pause():(q.src=je[t],q.volume=.5,q.play().catch(e=>console.error(`Audio play failed:`,e)))})}),l.querySelector(`.ambient-btn[data-sound="silence"]`).classList.add(`active`);let Me=document.getElementById(`poem-text`),J=document.getElementById(`highlight-tooltip`),Y=``;Me?.addEventListener(`mouseup`,()=>{let e=window.getSelection();if(!e.rangeCount||e.isCollapsed){J.classList.remove(`visible`);return}let t=e.getRangeAt(0).getBoundingClientRect();Y=e.toString().trim(),Y.length>0?(J.style.left=`${t.left+t.width/2}px`,J.style.top=`${t.top+window.scrollY}px`,J.classList.add(`visible`)):J.classList.remove(`visible`)}),document.addEventListener(`selectionchange`,()=>{window.getSelection().isCollapsed&&J.classList.remove(`visible`)}),document.getElementById(`highlight-copy-btn`)?.addEventListener(`click`,()=>{navigator.clipboard.writeText(Y).then(()=>{u.show(`Trecho copiado para a área de transferência!`,`success`),window.getSelection().removeAllRanges(),J.classList.remove(`visible`)})}),document.getElementById(`highlight-share-btn`)?.addEventListener(`click`,()=>{let e=`"${Y}" — Natanael Brentano\n${window.location.href}`,t=`https://twitter.com/intent/tweet?text=${encodeURIComponent(e)}`;window.open(t,`_blank`,`noopener,noreferrer`),window.getSelection().removeAllRanges(),J.classList.remove(`visible`)}),document.getElementById(`highlight-card-btn`)?.addEventListener(`click`,()=>{window.getSelection().removeAllRanges(),J.classList.remove(`visible`),b=!0,x=Y;let e=document.getElementById(`card-preview-modal`);e&&(e.style.display=`flex`,e.classList.add(`active`))}),y=e=>{if(![`INPUT`,`TEXTAREA`].includes(e.target.tagName))switch(e.key){case`Escape`:P&&L();break;case`ArrowRight`:C&&r(`/poema/${C}`);break;case`ArrowLeft`:S&&r(`/poema/${S}`);break;case`f`:case`F`:O?.click();break;case`i`:case`I`:P?L():Ce()}},document.addEventListener(`keydown`,y);let Ne=document.getElementById(`scroll-bar`),X=document.querySelector(`.poem-nav`),Z=document.getElementById(`next-btn`),Pe=!1;g=ae(()=>{let e=(document.body.scrollTop||document.documentElement.scrollTop)/(document.documentElement.scrollHeight-document.documentElement.clientHeight)*100;Ne&&(Ne.style.width=e+`%`);let t=window.pageYOffset||document.documentElement.scrollTop,n=document.documentElement.scrollHeight,r=document.documentElement.clientHeight;X&&(t>50?X.classList.add(`visible`):X.classList.remove(`visible`)),t+r>n*.9&&C&&!Pe&&(Pe=!0,Z&&(Z.style.transform=`scale(1.05)`),setTimeout(()=>{Z&&(Z.style.transform=`scale(1)`)},200))},100),window.addEventListener(`scroll`,g);let Fe=0,Ie=0;_=e=>{Fe=e.touches[0].clientX,Ie=e.touches[0].clientY},v=e=>{let t=Fe-e.changedTouches[0].clientX,n=Ie-e.changedTouches[0].clientY;Math.abs(t)>80&&Math.abs(t)>Math.abs(n)*2&&(t>0&&C?r(`/poema/${C}`):t<0&&S&&r(`/poema/${S}`))},document.addEventListener(`touchstart`,_,{passive:!0}),document.addEventListener(`touchend`,v,{passive:!0}),Z?.addEventListener(`click`,()=>{C&&r(`/poema/${C}`)}),document.getElementById(`prev-btn`)?.addEventListener(`click`,()=>{S&&r(`/poema/${S}`)}),te.init();let Le=document.getElementById(`share-card-btn`),Q=document.getElementById(`card-preview-modal`),Re=document.getElementById(`close-preview-btn`),$=document.getElementById(`download-card-btn`),ze=l.querySelectorAll(`.preview-theme-btn`),Be=l.querySelectorAll(`.preview-ratio-btn`),Ve=`dark`,He=`feed`;if(Le?.addEventListener(`click`,()=>{Q&&(Q.style.display=`flex`,Q.classList.add(`active`))}),Re?.addEventListener(`click`,()=>{Q&&(Q.style.display=`none`,Q.classList.remove(`active`))}),ze.forEach(e=>{e.addEventListener(`click`,()=>{ze.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),Ve=e.dataset.theme})}),Be.forEach(e=>{e.addEventListener(`click`,()=>{Be.forEach(e=>{e.classList.remove(`active`),e.style.borderColor=`var(--border-subtle)`}),e.classList.add(`active`),e.style.borderColor=`var(--border-strong)`,He=e.dataset.ratio})}),$?.addEventListener(`click`,async()=>{$.innerText=`Gerando...`,$.disabled=!0;try{let{generateSocialCard:e}=await t(async()=>{let{generateSocialCard:e}=await import(`./social-export-b3MWKN4-.js`);return{generateSocialCard:e}},__vite__mapDeps([0,1,2])),n=b?x:null;await e(m,document.getElementById(`social-card-container`),Ve,n,He),u.show(`Card gerado com sucesso!`,`success`)}catch(e){console.error(e),u.show(`Erro ao gerar card.`,`error`)}finally{$.innerText=`Baixar Imagem`,$.disabled=!1,b=!1,x=``,Q&&(Q.style.display=`none`,Q.classList.remove(`active`))}}),me){let e=document.getElementById(`resend-email-btn`);e&&e.addEventListener(`click`,async()=>{if(confirm(`Deseja realmente reenviar o email desta obra para todos os assinantes?`)){e.innerText=`Enviando...`,e.disabled=!0;try{let{data:e,error:t}=await i.functions.invoke(`send-newsletter`,{body:{poemId:m.id}});if(t)throw t;alert(`Email reenviado com sucesso para ${e?.count||0} assinantes!`)}catch(e){console.error(`Newsletter erro:`,e);let t=``;if(e.context&&typeof e.context.json==`function`)try{let n=await e.context.json();t=n.error||n.message||``}catch{}alert(`Houve um erro ao reenviar a newsletter:\n${t||e.message||`Erro na Edge Function`}`)}finally{e.innerText=`Reenviar Email`,e.disabled=!1}}})}setTimeout(()=>{[S,C].filter(Boolean).forEach(e=>{let t=document.createElement(`link`);t.rel=`prefetch`,t.href=`${window.location.origin}/poemas/poema/${e}`,document.head.appendChild(t)})},2e3)}};export{S as default};