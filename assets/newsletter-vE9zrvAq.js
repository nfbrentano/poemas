import{t as e}from"./supabase-C7rZ412s.js";var t={render(){return`
    ...
    `},init(){let t=document.getElementById(`subscribe-form`);t&&t.addEventListener(`submit`,async n=>{n.preventDefault();let r=document.getElementById(`subscriber-email`).value,i=document.getElementById(`subscribe-message`);if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(r)){i.innerHTML=`Por favor, insira um e-mail válido.`,i.style.color=`var(--error)`;return}i.innerHTML=`Enviando...`,i.style.color=`var(--text-secondary)`;let{error:a}=await e.from(`subscribers`).insert([{email:r}]);if(!a||a.code===`23505`){try{await fetch(`https://ejorjxvjglkkxnusdrzl.supabase.co/functions/v1/loops-subscribe`,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVqb3JqeHZqZ2xra3hudXNkcnpsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcwODU3NTcsImV4cCI6MjA5MjY2MTc1N30.4Y3xJlQblq4ZDCkkhaSgttuYvovMbctv1duzPG7frTI`},body:JSON.stringify({email:r})})}catch(e){console.warn(`Loops sync failed:`,e)}i.innerHTML=a?.code===`23505`?`Este e-mail já está inscrito.`:`Obrigado por assinar.`,i.style.color=a?.code===`23505`?`var(--text-secondary)`:`var(--success)`,a||t.reset()}else i.innerHTML=`Erro ao inscrever. Tente novamente.`,i.style.color=`var(--error)`})}};export{t};